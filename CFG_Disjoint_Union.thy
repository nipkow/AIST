...
proof -
  ...
  have "Lang ?P S \<subseteq> ?L1 @@ ?L2" 
    proof
      ...
      then have "?P \<turnstile> [Nt S] \<Rightarrow>* map Tm w" using CFG.Lang_def by fastforce
      ...
      then have "w1 \<in> ?L1" "w2 \<in> ?L2" unfolding CFG.Lang_def by simp_all
    ...
    proof
      ...
      then show "w \<in> Lang ?P S" unfolding CFG.Lang_def by auto
    qed
    ultimately show ?thesis by blast
qed
