...
definition (in Extended_Cfg) IPDA :: "(('n, 't) item, 't) npda" where
  "IPDA \<equiv> let
    P = Prods G';
    \<Delta> = {([[X \<rightarrow> \<beta> . Tm a # \<gamma>]], a, [[X \<rightarrow> \<beta> @ [Tm a] . \<gamma>]])|X \<beta> a \<gamma>. (X, \<beta> @ Tm a # \<gamma>) \<in> P};
    \<E> = {([[X \<rightarrow> \<beta> . Nt Y # \<gamma>]], [[Y \<rightarrow> [] . \<alpha>], [X \<rightarrow> \<beta> . Nt Y # \<gamma>]])
        | X \<beta> Y \<gamma> \<alpha>. (X, \<beta> @ Nt Y # \<gamma>) \<in> P \<and> (Y, \<alpha>) \<in> P} \<union> 
        {([[Y \<rightarrow> \<alpha> . []], [X \<rightarrow> \<beta> . Nt Y # \<gamma>]], [[X \<rightarrow> \<beta> @ [Nt Y] . \<gamma>]])
        | Y \<alpha> X \<beta> \<gamma>. (X, \<beta> @ Nt Y # \<gamma>) \<in> P \<and> (Y, \<alpha>) \<in> P}     
  in \<lparr>npda.states = It G', init = [S' \<rightarrow> [] . [Nt S]], final = {[S' \<rightarrow> [Nt S] . []]}, nxt = \<Delta>, eps = \<E>\<rparr>"

(* \<lparr>init = [S' \<rightarrow> [] . [Nt S]], init_symbol = [S' \<rightarrow> [] . []], 
      final = {[S' \<rightarrow> [Nt S] . []]}, nxt = \<Delta>, eps = \<E>\<rparr> *)
...
lemma nxt_ipda [simp]:
  "nxt M = {([[X \<rightarrow> \<beta> . Tm a # \<gamma>]], a, [[X \<rightarrow> \<beta> @ [Tm a] . \<gamma>]])|X \<beta> a \<gamma>. (X, \<beta> @ Tm a # \<gamma>) \<in> Prods G'}"
  using ipda unfolding IPDA_def by (meson select_convs(4))

lemma nxt_Tm_neq_empty [simp]:
  assumes "a \<noteq> b"
  shows "([X \<rightarrow> \<alpha> . Tm a # \<gamma>] # ps, b, q) \<notin> nxt M"
  using assms unfolding IPDA_def by simp

lemma nxt_nempty_imp_Tm_eq:
  assumes "(ps, a, qs) \<in> nxt M"
  obtains X \<beta> \<gamma> where "ps = [[X \<rightarrow> \<beta> . Tm a # \<gamma>]]" "(X, \<beta> @ Tm a # \<gamma>) \<in> Prods G'"
    "qs = [[X \<rightarrow> \<beta> @ [Tm a] . \<gamma>]]"
  using assms by auto

lemma nxt_nempty_imp_Tm_eq_Item:
  assumes "([[X \<rightarrow> \<beta> . Tm a # \<gamma>]], b, q) \<in>  nxt M"
  shows "(X, \<beta> @ Tm a # \<gamma>) \<in> Prods G'" "a = b"
  using nxt_nempty_imp_Tm_eq[OF assms] by blast+

lemma eps_ipda [simp]:
  "eps M =  {([[X \<rightarrow> \<beta> . Nt Y # \<gamma>]], [Y \<rightarrow> [] . \<alpha>] # [[X \<rightarrow> \<beta> . Nt Y # \<gamma>]])
...
lemma eps_cases [consumes 1, case_names expand reduce]:
  ...
  obtains X \<beta> Y \<gamma> \<alpha> where "ps = [[X \<rightarrow> \<beta> . Nt Y # \<gamma>]]" "qs = [[Y \<rightarrow> [] . \<alpha>], [X \<rightarrow> \<beta> . Nt Y # \<gamma>]]"
    "(X, \<beta> @ Nt Y # \<gamma>) \<in> Prods G'" "(Y, \<alpha>) \<in> Prods G'" |
    Y \<alpha> X \<beta> \<gamma> where "ps = [[Y \<rightarrow> \<alpha> . []], [X \<rightarrow> \<beta> . Nt Y # \<gamma>]]" "qs =  [[X \<rightarrow> \<beta> @ [Nt Y] . \<gamma>]]"
     "(X, \<beta> @ Nt Y # \<gamma>) \<in> Prods G'" "(Y, \<alpha>) \<in> Prods G'"
...
lemma shifting [simp]:
  ...
  shows "([A \<rightarrow> \<alpha> . Tm a # \<beta>]#\<rho>s, a#u) \<turnstile> ([A \<rightarrow> \<alpha>@[Tm a] . \<beta>]#\<rho>s, u)"
proof -
  have "([[A \<rightarrow> \<alpha> . Tm a # \<beta>]], a, [[A \<rightarrow> \<alpha> @ [Tm a] . \<beta>]]) \<in> nxt M"
...
proof -
  have "([[Y \<rightarrow> \<alpha> . []], [X \<rightarrow> \<beta> . Nt Y#\<gamma>]] @ \<rho>, w) \<turnstile> ([[X \<rightarrow> \<beta> @ [Nt Y] . \<gamma>]] @ \<rho>, w)"
...
lemma expanding_in_eps:
  assumes "(Y, \<alpha>) \<in> Prods G'" "(X, \<beta> @ Nt Y # \<gamma>) \<in> Prods G'" 
  shows "([[X \<rightarrow> \<beta> . Nt Y#\<gamma>]], [[Y \<rightarrow> [] . \<alpha>], [X \<rightarrow> \<beta> . Nt Y#\<gamma>]]) \<in> eps M"
  using assms by auto

lemma expanding:
  assumes "(Y, \<alpha>) \<in> Prods G'" "(X, \<beta> @ Nt Y # \<gamma>) \<in> Prods G'"
  shows "([X \<rightarrow> \<beta> . Nt Y#\<gamma>]#\<rho>, w) \<turnstile> ([Y \<rightarrow> [] . \<alpha>]#[X \<rightarrow> \<beta> . Nt Y#\<gamma>]#\<rho>, w)"
proof -
  have "([[X \<rightarrow> \<beta> . Nt Y#\<gamma>]] @ \<rho>, w) \<turnstile> ([[Y \<rightarrow> [] . \<alpha>], [X \<rightarrow> \<beta> . Nt Y#\<gamma>]] @ \<rho>, w)"
...
obtains A \<alpha> a \<beta> \<rho> u where 
    "c0 = ([A \<rightarrow> \<alpha> . Tm a # \<beta>]#\<rho>, a#u)" "c1 = ([A \<rightarrow> \<alpha>@[Tm a] . \<beta>]#\<rho>, u)" |
    ...
    Y \<alpha> X \<beta> \<gamma> \<rho> w where 
    "c0 = ([X \<rightarrow> \<beta> . Nt Y#\<gamma>]#\<rho>, w)"  "c1 = ([Y \<rightarrow> [] . \<alpha>]#[X \<rightarrow> \<beta> . Nt Y#\<gamma>]#\<rho>, w)"
  using assms by cases auto
...
lemma step_reaches_final_imp_S:
  ...
  shows "([A \<rightarrow> \<alpha> . \<beta>] # \<rho> @ \<sigma>, u) = ([S \<rightarrow> \<alpha> . \<beta>] # init M # \<sigma>, v)"
...
lemma completes_Tms:
  "(A, \<alpha> @ map Tm u @ \<beta>) \<in> Prods G' 
    \<Longrightarrow> ([A \<rightarrow> \<alpha> . map Tm u @ \<beta>]#is, u@v) \<turnstile>* ([A \<rightarrow> \<alpha> @ map Tm u . \<beta>]#is, v)"
proof (induction u arbitrary: \<alpha>)
  case (Cons a u)
  hence "([A \<rightarrow> \<alpha> . map Tm (a # u) @ \<beta>] #  is, (a # u) @ v) 
        \<turnstile> ([A \<rightarrow> \<alpha> @ [Tm a] . map Tm u @ \<beta>] # is, u @ v)"
    by simp
...
lemma init_expands[elim]:
  ...
  obtains \<alpha> where "(\<sigma>, v) = ([S \<rightarrow> [] . \<alpha>] # init M # \<rho>, u)"
    "(S, \<alpha>) \<in> Prods G"
...
lemma complete_S'_steps_refl:
  assumes "([[S' \<rightarrow> \<alpha> . []]], w) \<turnstile>* c"
  shows "([[S' \<rightarrow> \<alpha> . []]], w) = c"
...
lemma reachable_imp_init_or_in_G:
  ...
  obtains \<sigma> \<alpha> \<beta> where "\<rho> = \<sigma> @ [[S' \<rightarrow> \<alpha> . \<beta>]]" "\<alpha> @ \<beta> = [Nt S]"
...
proof -
  ...
  next
    ...
    from Suc.IH[OF this(1)] obtain \<tau> \<alpha> \<beta> where \<tau>_def:
      "\<sigma> = \<tau> @ [[S' \<rightarrow> \<alpha> . \<beta>]]" "\<alpha> @ \<beta> = [Nt S]" 
    ...
    next
      case (Cons i \<upsilon>)
      hence \<sigma>_def: "\<sigma> = i # \<upsilon> @ [[S' \<rightarrow> \<alpha> . \<beta>]]" using \<tau>_def by simp
      from n_steps(2)[unfolded this] show ?thesis 
      proof cases
        case (expand Y \<alpha>' X \<beta>' \<gamma> \<rho>' w')
        moreover with Cons have
          "\<rho> = [Y \<rightarrow> [] . \<alpha>'] # \<tau> @ [[S' \<rightarrow> \<alpha> . \<beta>]]" by simp
...
lemma first_step_is_eps:
  ...
  obtains \<alpha> where 
    "([init M], u) \<turnstile> ([[S \<rightarrow> [] . \<alpha>], init M], u)"
    "([[S \<rightarrow> [] . \<alpha>], init M], u) \<turnstile>* (qs, v)"
proof -
...
lemma reachable_snd_not_empty_imp_hd_in_G:
  assumes "([init M], u) \<turnstile>* ([A \<rightarrow> \<alpha> . \<beta>] # [B \<rightarrow> \<gamma> . \<delta>] # \<rho>, v)"
    "\<gamma>@\<delta> \<noteq> []"
  shows "(A, \<alpha>@\<beta>) \<in> Prods G"
proof -
  from reachable_imp_init_or_in_G[OF assms(1)] obtain \<sigma> \<alpha>' \<beta>' where \<sigma>_def:
    "[A \<rightarrow> \<alpha> . \<beta>] # [B \<rightarrow> \<gamma> . \<delta>] # \<rho> = \<sigma> @ [[S' \<rightarrow> \<alpha>' . \<beta>']]" "\<alpha>' @ \<beta>' = [Nt S]"
    "\<forall>i\<in>set \<sigma>. case i of [X \<rightarrow> \<gamma> . \<delta>] \<Rightarrow> (X, \<gamma> @ \<delta>) \<in> Prods G"
    by blast
  have "\<sigma> \<noteq> []" 
...
lemma singleton_derive_imp_completes:
  ...
  shows "([A \<rightarrow> \<alpha> . [Nt X] @ \<beta>] # \<rho>, u @ v) 
          \<turnstile>* ([A \<rightarrow> \<alpha> @ [Nt X] . \<beta>] # \<rho>, v)"
proof -
  note deriv = derive_singleton[of "Prods G'" "Nt X" "map Tm u"]
  with assms expanding have 
    "([A \<rightarrow> \<alpha> . [Nt X] @ \<beta>] # \<rho>, u @ v) 
      \<turnstile> ([X \<rightarrow> [] . map Tm u] # [A \<rightarrow> \<alpha> . [Nt X] @ \<beta>] # \<rho>, u @ v)"
    by auto
  also with completes_Tms step_imp_in_Prods have 
    "... \<turnstile>* ([X \<rightarrow> map Tm u . []] # [A \<rightarrow> \<alpha> . [Nt X] @ \<beta>] # \<rho>, v)" 
    by (metis deriv append.right_neutral append_Nil assms(1) completes_Tms sym.inject(1))
  also  have "... \<turnstile> ([A \<rightarrow> \<alpha> @ [Nt X] . \<beta>] # \<rho>, v)"
...
lemma derive_imp_completes:
  ...
  shows "([A \<rightarrow> \<alpha> . \<beta>@\<gamma>] # \<rho>, w @ x) \<turnstile>* ([A \<rightarrow> \<alpha>@\<beta> . \<gamma>] # \<rho>, x)"
proof -
  ...
  with completes_Tms[of A \<alpha> u "Nt X # map Tm y @ \<gamma>" _ "v @ y @ x"] have 
    "([A \<rightarrow> \<alpha> . \<beta> @ \<gamma>] # \<rho>, w @ x) 
      \<turnstile>* ([A \<rightarrow> \<alpha> @ map Tm u . Nt X # map Tm y @ \<gamma>] # \<rho>, v @ y @ x)" 
    using assms by simp
  also from singleton_derive_imp_completes[OF \<beta>_decomp(2), of _ "\<alpha> @ map Tm u" _ _ "y@x"] have 
    "... \<turnstile>* ([A \<rightarrow> \<alpha> @ map Tm u @ [Nt X] . map Tm y @ \<gamma>] # \<rho>, y @ x)"
    using \<beta>_decomp(1) assms(2) by auto
  also from completes_Tms have "... \<turnstile>* ([A \<rightarrow> \<alpha> @ \<beta> . \<gamma>] # \<rho>, x)"
...
proof -
  ...
  proof (induction n arbitrary: \<beta> w A \<alpha> \<gamma> \<rho> x rule: less_induct)
    ...
    proof (cases n)
      ...
      have first: "([A \<rightarrow> \<alpha> . \<beta> @ \<gamma>] # \<rho>, w @ x) 
              \<turnstile>* ([A \<rightarrow> \<alpha> @ \<delta>\<^sub>1 . Nt X # \<delta>\<^sub>2 @ \<gamma>] # \<rho>, v @ y @ x)"
      ...
      have last: "([A \<rightarrow> \<alpha> @ \<delta>\<^sub>1 @ [Nt X] . \<delta>\<^sub>2 @ \<gamma>] # \<rho>, y @ x) 
                  \<turnstile>* ([A \<rightarrow> \<alpha> @ \<beta> . \<gamma>] # \<rho>, x)"
      ...
      proof (cases "j = n")
        ...
        proof (cases m)
          ...
          from m_steps \<beta>'_decomp(1) have X_step: "(X, \<xi>\<^sub>1 @ Nt Y # \<xi>\<^sub>2) \<in> Prods G'" 
            "([A \<rightarrow> \<alpha> @ \<delta>\<^sub>1 . Nt X # \<delta>\<^sub>2 @ \<gamma>] # \<rho>, v @ y @ x) 
...
lemma reaches_final_imp_complete_reaches_final:
  assumes "([A \<rightarrow> \<alpha> . \<beta>] # \<rho>, w) \<turnstile>(n) ([final_state], [])"
  obtains u v m k where
    "m + k = n"
    "w = u @ v"
    "Prods G' \<turnstile> \<beta> \<Rightarrow>* map Tm u"
    "([A \<rightarrow> \<alpha> . \<beta>] # \<rho>, w) \<turnstile>(m) ([A \<rightarrow> \<alpha> @ \<beta> . []] # \<rho>, v)"
    "([A \<rightarrow> \<alpha> @ \<beta> . []] # \<rho>, v) \<turnstile>(k) ([final_state], [])"
  using assms proof (induction n arbitrary: A \<alpha> \<beta> \<rho> w thesis rule: less_induct)
  case (less n)
  show ?case 
  proof (cases n)
    case (Suc m)
    then obtain \<sigma> x where step: "([A \<rightarrow> \<alpha> . \<beta>] # \<rho>, w) \<turnstile> (\<sigma>, x)"
    ...
    from step obtain B \<gamma> \<delta> \<tau> u v j k where \<sigma>_complete:
      "\<sigma> = [B \<rightarrow> \<gamma> . \<delta>] # \<tau>" "x = u @ v" "Prods G' \<turnstile> \<delta> \<Rightarrow>* map Tm u"
      "([B \<rightarrow> \<gamma> . \<delta>] # \<tau>, u @ v) \<turnstile>(j) ([B \<rightarrow> \<gamma> @ \<delta> . []] # \<tau>, v)"
      "([B \<rightarrow> \<gamma> @ \<delta> . []] # \<tau>, v) \<turnstile>(k) ([final_state], [])"
      "j + k = m" using less.IH 
    ...
    proof cases
      ...
      with shift have 
        "([A \<rightarrow> \<alpha> . \<beta>] # \<rho>, w) \<turnstile> ([A \<rightarrow> \<alpha> @ [Tm a] . \<beta>'] # \<rho>, u @ v)"
        using step by auto
      also have "... \<turnstile>(j) ([A \<rightarrow> \<alpha> @ \<beta> . []] # \<rho>, v)" 
    ...
    next
      ...
      from eqs expand have exp_step:
        "([A \<rightarrow> \<alpha> . \<beta>] # \<rho>, w) 
          \<turnstile> ([Y \<rightarrow> [] . \<gamma>'] # [A \<rightarrow> \<alpha> . Nt Y # \<beta>'] # \<rho>, u @ v)"
        using step by auto
      moreover with \<sigma>_complete eqs have j_steps: 
        "... \<turnstile>(j) ([Y \<rightarrow> \<gamma>' . []] # [A \<rightarrow> \<alpha> . Nt Y # \<beta>'] # \<rho>, v)"
        using expand by simp
      moreover have reduct_step: "... \<turnstile> ([A \<rightarrow> \<alpha> @ [Nt Y] . \<beta>'] # \<rho>, v)"
      ...
      moreover with less.IH obtain v' x' j' k' l where complete_reaches: "v = v' @ x'" "Prods G' \<turnstile> \<beta>' \<Rightarrow>* map Tm v'"
        "... \<turnstile>(j') ([A \<rightarrow> \<alpha> @ \<beta> . []] # \<rho>, x')"
        "([A \<rightarrow> \<alpha> @ \<beta> . []] # \<rho>, x') \<turnstile>(k') ([final_state], [])"
        "k = Suc l"
        "j' + k' = l"
      proof - (* TODO refactor *)
        from expand step \<sigma>_complete eqs have 
          "([Y \<rightarrow> \<gamma>' . []] # [A \<rightarrow> \<alpha> . Nt Y # \<beta>'] # \<rho>, v) 
            \<turnstile>(k) ([final_state], [])"
          by auto
        moreover have "[Y \<rightarrow> \<gamma>' . []] # [A \<rightarrow> \<alpha> . Nt Y # \<beta>'] # \<rho> 
          \<noteq> [final_state]"  by auto
        ultimately obtain l where l_steps: "k = Suc l"
          "([A \<rightarrow> \<alpha> @ [Nt Y] . \<beta>'] # \<rho>, v) \<turnstile>(l) ([final_state], [])"
      ...
      qed
      ultimately have A_completes: "([A \<rightarrow> \<alpha> . \<beta>] # \<rho>, w) 
          \<turnstile>(Suc (Suc j) + j') ([A \<rightarrow> \<alpha> @ \<beta> . []] # \<rho>, x')"
...
lemma reaches_final_imp_completes:
  assumes "([A \<rightarrow> \<alpha> . \<beta>] # \<rho>, w) \<turnstile>* ([final_state], [])"
  obtains u v where 
    "w = u @ v"
    "Prods G' \<turnstile> \<beta> \<Rightarrow>* map Tm u"
    "([A \<rightarrow> \<alpha> . \<beta>] # \<rho>, w) \<turnstile>* ([A \<rightarrow> \<alpha> @ \<beta> . []] # \<rho>, v)"
    "([A \<rightarrow> \<alpha> @ \<beta> . []] # \<rho>, v) \<turnstile>* ([final_state], [])"
proof -
...
lemma reaches_final_imp_second_is_chain:
  assumes "([A \<rightarrow> \<alpha> . \<beta>] # i # \<rho>, w) \<turnstile>* ([final_state], [])"
  obtains X \<alpha>' \<beta>' where "i = [X \<rightarrow> \<alpha>' . Nt A # \<beta>']"
proof -
  from assms reaches_final_imp_completes obtain x where 
    "([A \<rightarrow> \<alpha> @ \<beta> . []] # i # \<rho>, x) \<turnstile>* ([final_state], [])"
    by metis
  then show thesis
...
lemma reaches_without_stack_imp_S':
  assumes "([[A \<rightarrow> \<alpha> . \<beta>]], w) \<turnstile>* ([final_state], [])"
  shows "[A \<rightarrow> \<alpha> . \<beta>] = init M \<or> [A \<rightarrow> \<alpha> . \<beta>] = final_state"
proof -
  from reaches_final_imp_completes[of _ _ _ "[]"] assms obtain u v where "w = u @ v"
    "Prods G' \<turnstile> \<beta> \<Rightarrow>* map Tm u"
    "([[A \<rightarrow> \<alpha> . \<beta>]], w) \<turnstile>* ([[A \<rightarrow> \<alpha> @ \<beta> . []]], v)"
    "([[A \<rightarrow> \<alpha> @ \<beta> . []]], v) \<turnstile>* ([final_state], [])"
...
lemma deriver_imp_IPDA_comp:
  ...
  shows
    "([[S' \<rightarrow> \<alpha> . \<beta>]], v) \<turnstile>* ([final_state], [])"
proof -
