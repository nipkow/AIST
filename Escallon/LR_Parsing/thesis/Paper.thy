...
Finally, if premises \<open>A\<^sub>1, A\<^sub>2, \<dots>, A\<^sub>n\<close> imply conclusion \<open>B\<close>, we write \[ \<open>\<lbrakk>A\<^sub>1; A\<^sub>2; \<dots>; A\<^sub>n\<rbrakk> \<Longrightarrow> B\<close> \]\<close>
...
@{term [display] \<open>It G = (\<Union>(A,w)\<in>Prods G. {[A \<rightarrow> \<alpha> \<cdot> \<beta>] | \<alpha> \<beta>. \<alpha>@\<beta> = w})\<close>}
...
\begin{multline}
  ...
  \<open>| X \<beta> Y \<gamma> \<alpha>. (X, \<beta> @ Nt Y # \<gamma>) \<in> Prods G' \<and> (Y, \<alpha>) \<in> Prods G'}\<close> \mathrm{,\ and}
\end{multline}
...
@{term \<open>Prods G' \<turnstile> [Nt S'] \<Rightarrow>r* map Tm w\<close>}:\par
If the current topmost stack item is @{term \<open>[A \<rightarrow> \<alpha> \<cdot> Tm a # \<beta>]\<close>} for any \<open>a :: 't\<close>, \<open>I\<^sub>G\<close> will 
invariably shift \<open>Tm a\<close>, effectively replacing this topmost item by @{term \<open>[A \<rightarrow> \<alpha> @ [Tm a] \<cdot> \<beta>]\<close>}. 
...
@{term \<open>[X \<rightarrow> \<beta> @ [Nt Y] \<cdot> \<gamma>]\<close>} is equivalent to the backwards (i.e. from right to left)
application of the rightmost derivation step:

\[ @{prop \<open>Prods G' \<turnstile> \<beta> @ Nt Y # map Tm u \<Rightarrow>r \<beta> @ \<alpha> @ map Tm u\<close>} \mathrm{\ for\ some\ } \<open>u\<close> \]
...
expands 
\[ @{term \<open>[X \<rightarrow> \<beta> \<cdot> Nt Y # \<gamma>] # \<rho>\<close>} \] 
to \mbox{@{term \<open>[Y \<rightarrow> \<cdot> \<alpha> ] # [X \<rightarrow> \<beta> \<cdot> Nt Y # \<gamma>] # \<rho>\<close>}}, 
it will only complete \<open>\<alpha>\<close>, i.e., reach @{term \<open>[Y \<rightarrow> \<alpha> \<cdot> ] # [X \<rightarrow> \<beta> \<cdot> Nt Y # \<gamma>] # \<rho>\<close>}, if for some 
prefix \<open>u\<close> of the remaining input word \<open>w = uv\<close>, @{prop \<open>Prods G' \<turnstile> \<alpha> \<Rightarrow>* map Tm u\<close>} holds.
 
We will now work towards proving that \<open>I\<^sub>G\<close> accepts exactly \<open>LangS G\<close>.
...
\begin{quote}
\<open>(\<rho>, u @ v) \<turnstile>* ([A \<rightarrow> \<alpha> \<cdot> Tm a # \<beta>] # \<sigma>, a # v) \<turnstile> (\<tau>, v)\<close>,
\end{quote}
...
\[ \<open>[init M] = rev \<rho> = [[S' \<rightarrow> \<cdot> [Nt S]]]\<close> \mathrm{\ and \ } @{prop \<open>u @ v = v\<close>} \] 
hold. This in turn implies @{prop \<open>hist \<rho> = []\<close>} and @{prop \<open>u = []\<close>}. Since
\mbox{@{prop \<open>Prods G \<turnstile> [] \<Rightarrow>* []\<close>}} holds, the invariant holds.\par
...
@{term \<open>(rev \<sigma>, w) = ([A \<rightarrow> \<alpha> \<cdot> Tm a # \<beta>] # \<tau>, a # v)\<close>}\mathrm{\ and}
\end{equation}
\begin{equation}\label{eq:ipda.invariant.rho_shift}
@{term \<open>rev \<rho> = [A \<rightarrow> \<alpha> @ [Tm a] \<cdot> \<beta>] # \<tau>\<close>}
\end{equation}

With Corollary~\ref{steps_shift_decomp}, this implies the existence of some \<open>y :: 't list\<close> such that
\mbox{\<open>u = ya\<close>}. This, together with (\ref{eq:ipda.invariant.stepn}), (\ref{eq:ipda.invariant.shift}), and 
...
This, \<open>u = ya\<close> and (\ref{eq:ipda.invariant.ih_shift}) imply 
...
@{term \<open>(rev \<sigma>, w) = ([Y \<rightarrow> \<alpha> \<cdot> ] # [X \<rightarrow> \<beta> \<cdot> Nt Y # \<gamma>] # \<tau>, v)\<close>}\mathrm{\ and}
\end{equation}
\begin{equation}\label{eq:ipda.invariant.rho_reduce}
@{term \<open>rev \<rho> = [X \<rightarrow> \<beta> @ [Nt Y] \<cdot> \<gamma>] # \<tau>\<close>}
\end{equation}

for some \<open>Y, \<alpha>, X, \<beta>, \<gamma>\<close> and \<open>\<tau>\<close>. By Lemma~\ref{reducing_imp_in_Prods_G}, we know that 
\mbox{@{prop \<open>(Y, \<alpha>) \<in> Prods G\<close>}}. Furthermore, (\ref{eq:ipda.invariant.reduce}) and 
(\ref{eq:ipda.invariant.rho_reduce}) imply @{prop \<open>hist \<rho> = hist (rev \<tau>) @ \<beta> @ [Nt Y]\<close>} and 
...
Lastly, @{prop \<open>Prods G \<turnstile> hist \<rho> \<Rightarrow>* map Tm u\<close>} follows from this and 
(\ref{eq:ipda.invariant.reduce_rs}) by transitivity, fulfilling the invariant.

Finally, in the expanding case we have 
\[ \<open>(rev \<sigma>, w) = ([X \<rightarrow> \<beta> \<cdot> Nt Y # \<gamma>] # \<tau>, v)\<close> \mathrm{\ and} \]
...
\[ \<open>hist \<rho> = hist \<sigma>\<close> \mathrm{\ and \ }  \<open>Prods G \<turnstile> hist \<sigma> \<Rightarrow>* map Tm u\<close> \]
...
\[ \<open>([init I\<^sub>G], w) =\<close>\ @{prop \<open>([init I\<^sub>G], w @ [])  \<turnstile>* ([[S' \<rightarrow> [Nt S] \<cdot> ]], [])\<close>} \] By 
Lemma~\ref{ipda.invariant}, this implies @{prop \<open>Prods G \<turnstile> hist [final_state] \<Rightarrow>* map Tm w\<close>}.
...
If @{prop \<open>Prods G' \<turnstile> \<beta> \<Rightarrow>* map Tm w\<close>} and @{prop \<open>(A, \<alpha> @ \<beta> @ \<gamma>) \<in> Prods G'\<close>}, then for any 
\<open>\<rho>, x\<close> holds: 
...
@{prop \<open>Prods G' \<turnstile> \<delta>\<^sub>1 \<Rightarrow>(i) map Tm u\<close>}\\
@{prop \<open>Prods G' \<turnstile> [Nt X] \<Rightarrow>(j) map Tm v\<close>}\\
@{prop \<open>Prods G' \<turnstile> \<delta>\<^sub>2 \<Rightarrow>(k) map Tm y\<close>}\\
...
If \<open>j = n\<close>, then \<open>i = j = 0\<close> and therefore
 
\begin{equation}\label{d_imp_c.d1u_d2y}
\<open>\<delta>\<^sub>1 = map Tm u\<close>\ \mathrm{and\ } \<open>\<delta>\<^sub>2 = map Tm y\<close>
\end{equation} 

hold. \<open>j = n\<close> also
implies the existence of some \<open>\<beta>' :: syms\<close> such that 

\begin{equation}\label{eq:d_imp_c.stepm}
\<open>Prods G' \<turnstile> [Nt X] \<Rightarrow> \<beta>' \<Rightarrow>(m) map Tm v\<close>
\end{equation}

We now distinguish yet another two cases, now on \<open>m\<close>. From now on in this proof, instead of 
@{term \<open>map Tm z\<close>} we occasionally write @{term \<open>z\<close>} for \<open>z :: 't list\<close>. We will also sometimes 
concatenate lists without an explicit operator, meaning we write @{term \<open>\<alpha> @ \<beta>\<close>} as \<open>\<alpha>\<beta>\<close>. 
Furthermore, $\<open>\<turnstile>\<^sub>S\<close>$, $\<open>\<turnstile>\<^sub>R\<close>$, and $\<open>\<turnstile>\<^sub>E\<close>$ denote shifting, reducing, and expanding transitions 
respectively.\par

If \<open>m = 0\<close>, then @{prop \<open>Prods G' \<turnstile> [Nt X] \<Rightarrow> v\<close>}. By Lemma~\ref{completes_Tms} 
(L\ref{completes_Tms}) we have:

\begin{align*}
&@{term \<open>([A \<rightarrow> \<alpha> \<cdot> \<beta>\<gamma>] # \<rho>, wx)\<close>}\\
&\;\overset{(\ref{d_imp_c.b_decomp(1)}, \ref{d_imp_c.b_decomp(2)}, \ref{d_imp_c.d1u_d2y})}{=\ }
  \<open>([A \<rightarrow> \<alpha> \<cdot> u @ Nt X # y\<gamma>] # \<rho>, uvyx)\<close>\\
&\;\overset{(L\ref{completes_Tms})}{\<open>\<turnstile>*\<close>\ } 
  \<open>([A \<rightarrow> \<alpha>u \<cdot> Nt X # y\<gamma>] # \<rho>, vyx)\<close>\\
&\;\<open>\<turnstile>\<^sub>E ([X \<rightarrow> \<cdot> v] # [A \<rightarrow> \<alpha>u \<cdot> Nt X # y\<gamma>] # \<rho>, vyx)\<close>\\
&\;\overset{(L\ref{completes_Tms})}{\<open>\<turnstile>*\<close>\ }
  \<open>([X \<rightarrow> v \<cdot> ] # [A \<rightarrow> \<alpha>u \<cdot> Nt X # y\<gamma>] # \<rho>, yx)\<close>\\
&\;\<open>\<turnstile>\<^sub>R ([A \<rightarrow> \<alpha>u[Nt X] \<cdot> y\<gamma>] # \<rho>, yx)\<close>\\
&\;\overset{(L\ref{completes_Tms})}{\<open>\<turnstile>*\<close>\ }
  \<open>([A \<rightarrow> \<alpha>u[Nt X]y \<cdot> \<gamma>] # \<rho>, x)\<close> 
...
@{prop \<open>Prods G' \<turnstile> \<xi>\<^sub>1 \<Rightarrow>(i') map Tm u'\<close>}\\
@{prop \<open>Prods G' \<turnstile> [Nt Y] \<Rightarrow>(j') map Tm v'\<close>}\\
...
@{prop \<open>Prods G' \<turnstile> [Nt Y] \<Rightarrow>(j') map Tm v'\<close>} implies that @{prop \<open>j' > 0\<close>}, i.e., 
@{prop \<open>j' = Suc j''\<close>} for some \<open>j''\<close>, meaning this derivation of \<open>v'\<close> is of the form 

\begin{equation}\label{eq:d_imp_c.stepj''}
\<open>Prods G' \<turnstile> [Nt Y] \<Rightarrow> \<gamma>' \<Rightarrow>(j'') map Tm v'\<close> \mathrm{\ for\ some\ } \<open>\<gamma>' :: syms\<close>
\end{equation}

Since @{prop \<open>i' < n\<close>}, @{prop \<open>j'' < n\<close>} and @{prop \<open>k' < n\<close>}, we can use the Induction Hypothesis
...
&\;\overset{(\ref{d_imp_c.b_decomp(1)}, \ref{d_imp_c.b_decomp(2)}, \ref{d_imp_c.d1u_d2y})}{=\ }
  \<open>([A \<rightarrow> \<alpha> \<cdot> u @ Nt X # y\<gamma>] # \<rho>, uvyx)\<close>\\
&\;\overset{(L\ref{completes_Tms})}{\<open>\<turnstile>*\<close>\ } \<open>([A \<rightarrow> \<alpha>u \<cdot> Nt X # y\<gamma>] # \<rho>, vyx)\<close>\\
&\;\overset{(\ref{eq:d_imp_c.stepm})}{\<open>\<turnstile>\<^sub>E\<close>\ } \<open>([X \<rightarrow> \<cdot> \<beta>'] # [A \<rightarrow> \<alpha>u \<cdot> Nt X # y\<gamma>] # \<rho>, vyx)\<close>\\
&\;\overset{(\ref{d_imp_c.b'_decomp(1)}, \ref{d_imp_c.b'_decomp(2)})}{=\ }
  \<open>([X \<rightarrow> \<cdot> \<xi>\<^sub>1 @ Nt Y # \<xi>\<^sub>2] # [A \<rightarrow> \<alpha>u \<cdot> Nt X # y\<gamma>] # \<rho>, u'v'y'yx)\<close>\\
&\;\overset{(IH)}{\<open>\<turnstile>*\<close>\ } \<open>([X \<rightarrow> \<xi>\<^sub>1 \<cdot> Nt Y # \<xi>\<^sub>2] # [A \<rightarrow> \<alpha>u \<cdot> Nt X # y\<gamma>] # \<rho>, v'y'yx)\<close>\\
&\;\overset{(\ref{eq:d_imp_c.stepj''})}{\<open>\<turnstile>\<^sub>E\<close>\ } 
  \<open>([Y \<rightarrow> \<cdot> \<gamma>'] [X \<rightarrow> \<xi>\<^sub>1 \<cdot> Nt Y # \<xi>\<^sub>2] # [A \<rightarrow> \<alpha>u \<cdot> Nt X # y\<gamma>] # \<rho>, v'y'yx)\<close>\\
&\;\overset{(IH)}{\<open>\<turnstile>*\<close>\ } \<open>([Y \<rightarrow> \<gamma>' \<cdot>] [X \<rightarrow> \<xi>\<^sub>1 \<cdot> Nt Y # \<xi>\<^sub>2] # [A \<rightarrow> \<alpha>u \<cdot> Nt X # y\<gamma>] # \<rho>, y'yx)\<close>\\
&\; \<open>\<turnstile>\<^sub>R ([X \<rightarrow> \<xi>\<^sub>1[Nt Y] \<cdot> \<xi>\<^sub>2] # [A \<rightarrow> \<alpha>u \<cdot> Nt X # y\<gamma>] # \<rho>, y'yx)\<close>\\
&\;\overset{(IH)}{\<open>\<turnstile>*\<close>\ } \<open>([X \<rightarrow> \<xi>\<^sub>1[Nt Y]\<xi>\<^sub>2 \<cdot> ] # [A \<rightarrow> \<alpha>u \<cdot> Nt X # y\<gamma>] # \<rho>, yx)\<close>\\
&\; \<open>\<turnstile>\<^sub>R ([A \<rightarrow> \<alpha>u[Nt X] \<cdot> y\<gamma>] # \<rho>, yx)\<close>\\
&\;\overset{(L\ref{completes_Tms})}{\<open>\<turnstile>*\<close>\ }
  \<open>([A \<rightarrow> \<alpha>u[Nt X]y \<cdot> \<gamma>] # \<rho>, x)\<close> 
\overset{(\ref{d_imp_c.b_decomp(1)}, \ref{d_imp_c.d1u_d2y})}{=\ } \<open>([A \<rightarrow> \<alpha>\<beta> \<cdot> \<gamma>] # \<rho>, x)\<close>
...
Therefore, the implication also holds in the case where @{prop \<open>j = n\<close>}. We now consider the case 
where @{prop \<open>j \<noteq> n\<close>}:\par

With @{prop \<open>n = i + j + k\<close>} and @{prop \<open>j > 0\<close>}, we know that @{prop \<open>i < n\<close>}, @{prop \<open>j < n\<close>}, and
@{prop \<open>k < n\<close>} all hold. Therefore, we can use the IH for each of the derivations in the 
decomposition:

\begin{align*}
&@{term \<open>([A \<rightarrow> \<alpha> \<cdot> \<beta>\<gamma>] # \<rho>, wx)\<close>}\\
&\;\<open>= ([A \<rightarrow> \<alpha> \<cdot> \<delta>\<^sub>1 @ Nt X # \<delta>\<^sub>2\<gamma>] # \<rho>, uvyx)\<close>\\
&\;\overset{(IH)}{\<open>\<turnstile>*\<close>\ } \<open>([A \<rightarrow> \<alpha>\<delta>\<^sub>1 \<cdot> Nt X # \<delta>\<^sub>2\<gamma>] # \<rho>, vyx)\<close>\\
&\;\overset{(IH)}{\<open>\<turnstile>*\<close>\ } \<open>([A \<rightarrow> \<alpha>\<delta>\<^sub>1 @ [Nt X] \<cdot> \<delta>\<^sub>2\<gamma>] # \<rho>, yx)\<close>\\
&\;\overset{(IH)}{\<open>\<turnstile>*\<close>\ } \<open>([A \<rightarrow> \<alpha>\<delta>\<^sub>1[Nt X]\<delta>\<^sub>2 \<cdot> \<gamma>] # \<rho>, x) = ([A \<rightarrow> \<alpha>\<beta> \<cdot> \<gamma>] # \<rho>, x)\<close>\\
\end{align*}
...
\begin{quote}
For each derivation \<open>Prods G \<turnstile> [Nt A] \<Rightarrow> \<alpha> \<Rightarrow>* map Tm w\<close>, 
\mbox{@{prop \<open>([A \<rightarrow> \<cdot> \<alpha>] # \<rho>, w @ v) \<turnstile>* ([A \<rightarrow> \<alpha> \<cdot> ] # \<rho>, v)\<close>}} for arbitrary \<open>\<rho> :: item list\<close> and 
\<open>v :: 't list\<close>.
\end{quote}
