...
\begin{abstract}
This paper starts from a recent formal verification
...
Everything has been formalized in Isabelle/HOL.
\end{abstract}
...
This was the starting point of the work by Nipkow and Rau \cite{NipkowR-CBJ24}.
We continue their work as follows.
First we refine the recognizer from sets to lists, then from inductive to recursive, and finally
we augment the lists with a more efficient array-like data structure.
...
Their work is more monolithic whereas ours aims for simple refinement steps and proofs.
%Their parser correctness theorems consider only a single parse tree.

This paper is largely based on the master's thesis by the second author \cite{Haberl}.

There are a number of related publications on verified parsing algorithms and parser generators
for a variety of grammars and formalisms:
for SLR grammars \cite{BarthwalN09},
parsing expression grammars \cite{KoprowskiB-LMiCS11,BlaudeauS-CPP20},
parser combinators \cite{Ridge-CPP11},
LL(1) grammars \cite{LasserCFR-ITP19} and
non-left-recursive grammars \cite{LasserCFR-PLDI21}.
There is also a verified validator for LR(1) automata \cite{JourdanPL-ESOP12}.
What sets our work apart is the algorithm and the complexity analysis.
