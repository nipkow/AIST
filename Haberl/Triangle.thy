...
proof(cases "As = []")
  ...
  (*have "dep_on (solve_tri (A # As) (A' # As') R) A \<inter> ({A} \<union> set As) = dep_on (solve_tri [A] (A' # As') R) A \<inter> {A}" using True by simp
  also have "... = dep_on (solve_lrec A A' (exp_hd A [] (solve_tri [] As' R))) A \<inter> {A}" by simp
  also have "... = dep_on (solve_lrec A A' R) A \<inter> {A}" by auto
  also have "... = {}" using solve_lrec_no_own_dep assms by fastforce
  finally show ?thesis .*)
next
  case False
