...
All our types are parameterized by type variables \<open>'n\<close> and \<open>'t\<close> which we use for the types of \concept{nonterminals} and \concept{terminals}.
...
The function \<^const>\<open>set\<close> converts in one direction, but we cannot convert from sets to lists
...
the proof of the Chomsky--Sch\"utzenberger Theorem.

Sometimes we need to generate fresh names, which requires that the underlying type
is infinite. We assume this via the use of type classes but elide it in our presentation.
...
\end{quote}
Our proof is based partly on the non-constructive proof by Barthwal and Norrish \cite{csl/BarthwalN10}.
%Note: What exactly does \<open>another\<close> mean in the next sentence? Maybe one of 
the following is correct (i don't know): \<open>similar\<close>, \<open>different\<close>
...
\subsection{Pushdown Automata} %Note: Why not name the author of the lean formalization?
...
We have shown that every right-linear grammar (this means productions have the form \<open>A \<rightarrow> wB\<close> or \<open>A \<rightarrow> w\<close>
where \<open>w\<close> is a terminal word) can be transformed into a strongly right-linear one
(which means that productions have the form \<open>A \<rightarrow> aB\<close> or \<open>A \<rightarrow> \<epsilon>\<close> where \<open>a\<close> is a terminal symbol).
...
\end{definition}
In this section we describe the executable function @{const GNF_of},
...
%Note: Do both references contain both algorithms? Or is 2nd step in Harrison and the 3rd step in Hopcroft? In that case write \<open>follow *the* textbook algorithms\<close>
%Note : Since you do not require the input to be in CNF, did you need to change the algorithms? 
% Or is this a superfluous assumptions in the textbooks? 
% Or do the textbooks describe a full GNF algorithm and we only take a part from that? please include the answer to this in the paper.
The last two steps follow textbook algorithms~\cite{Harrison78,HopcroftU79} for deriving a GNF,
but we do not require the input to be in CNF in our formalization. We only require the input to contain no \<open>\<epsilon>\<close>-productions.
As a result we arrive at a head GNF.

% i guess it is intentional that *on* is bold?
...
there exist no \<open>A \<rightarrow> B\<alpha> \<in> P\<close> with @{prop \<open>B \<in> set As\<close>}.

We recursively turn a grammar that is triangular on \<open>As\<close>
into a grammar triangular on \<open>A#As\<close>.
First, we expand productions of form \<open>A \<rightarrow> B\<alpha>\<close> for all @{prop \<open>B \<in> set As\<close>}
...
%Note: instead of \<open>solved\<close> maybe \<open>eliminated\<close> or \<open>processed\<close>?
Afterwards productions of the form \<open>A \<rightarrow> Av\<close> remain to be solved.
Let \<open>V\<close> collect all such \<open>v\<close>,
and let \<open>U\<close> collect all \<open>u\<close> of productions \<open>A \<rightarrow> u\<close> that do not start with \<open>A\<close>.
% "The language of \<open>A\<close>" i guess means that one uses A as start symbol?
...
transforms an \<open>\<epsilon>\<close>-free grammar into triangular form:
\begin{quote}
...
\item the nonterminals of \<open>P\<close> are in \<open>As\<close> (@{thm(prem 4) Triangular_Solve_tri}).
\end{itemize}
...
first makes fresh copies \<open>As'\<close> of the nonterminals in \<open>As\<close>,
...
We close this section by considering the size of the produced grammar. Like
...
% is this Paulsons NFA's? If not, why did you need to formalize it again?
% Also what about the Todo above this note?
...
over a single letter alphabet is already regular.
...
%Note: Would it maybe be more accurate to say that one expresses a %context-free-grammar as a system of inequalities such that any minimal %solution to the system induces the Parikh-same language and one constructs a %minimal solution that happens to induce a regular language?
% I ask, because a language being a solution of your inequalities seems very inaccurate. 
%If you decide to change this, also consider adapting the very last paragraph of section 6, which is also about the minimality and uniqueness stuff.
...
By constructing a regular language which also is a minimal solution to the same system of inequalities,
...
Furthermore, given \<open>s ::\<close> @{typ "nat \<Rightarrow> 'a rlexp"}, @{term "subst s r"} denotes the regular language expression which we obtain
...
\concept{regularity preserving}, denoted by @{term "reg_eval r"} in Isabelle.
...
\end{quote}
%What is the notion of less-than/minimality for a valuation? This is not obvious and should be in the paper.
...
%Note: where is the condition that \<open>sys\<close> is induced by the cfg in this lemma?
\begin{lemma}
...
Following Pilling's proof, we first construct a regular, minimal solution to a system
...
%Why does Pilling claim equality but you do not prove it? Is it not needed for the final result? Is the \<open>generalization\<close> in the next paragraph your way around this? 
