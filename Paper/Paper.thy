...
\subsection{From @{term \<open>L\<close>} to @{term \<open>L'\<close>}}
...
We proceded proving the \<open>\<subseteq>\<close> direction of @{prop "L = \<h> ` (R \<inter> Dyck_lang \<Gamma>)"} by proving that the regular conditions @{const P1} -- @{const P5} keep holding in each derivation step.
