...
However, some constructions are hard or impossible even for finite \<open>Prods\<close>,
unless we have some order on them: if we need to create new variables that appear in the output,
the order in which a \<open>Prods\<close> is processed is crucial. Therefore we work with \emph{lists} of
...
@{thm Lang_def}
\end{quote}
There is also the abbreviation @{abbrev "lang ps A"}. We do not present all these obvious abbreviations
but merely state that if there is a function \<open>Fn\<close> operating on sets, the lower case \<open>fn\<close>
is its analogue on lists. 
