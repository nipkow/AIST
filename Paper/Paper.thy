...
\]
The strategy will be, to impose enough structure on $L$, attaining a Language $L'$, such that the words of $L'$ can be identified using only regular properties $R$ and the information that it's words are now balanced words of brackets over @{term \<open>\<Gamma>\<close>}. We can then delete all this new structure again -- using the homomorphism @{term \<open>\<h>\<close>} -- to arrive at the original language $L$.
...
This gives us an easy to understand definition of balanced words, but it makes it hard to prove certain lemmas about balanced substrings. In particular the following lemma is needed for our proof of the Chomsky Schuetzenberger theorem:
\[
    @{thm bal_Open_split}.
\]
In order to prove this, we further provide a function based characterization of balanced words: @{term \<open>bal_stk\<close>}. It uses a stack, to remember which opening bracket is to be closed next.

Since we often work with mixed words, we further need a generalization @{term \<open>bal_tm\<close>} that ignores any nonterminals that might occur in the word for the balancedness check. For finished words, the two agree. Since the letters in mixed words are marked by the type constructors @{term \<open>Tm\<close>} or @{term \<open>Nt\<close>} this agreement spells out to
\[
    @{thm bal_iff_bal_tm}.
\]
%
\subsection{From $L$ to $L'$}
For most of the proof, we assume that our grammar $P$ for $L$ is in chomsky normal form. This assumption is lifted only at the very end. 
We use the cnf form to encode the parse tree -- i.e. which productions were used -- into the words of $L'$ by replacing
% 
\begin{align*}
   &\pi = A \rightarrow BC \,\text{ by }\, \pi' = A \rightarrow [^1_\pi \, B \, ]^1_\pi \, [^2_\pi \, C \, ]^2_\pi \\
   \text{ and } 
...
First, we again need a generalized version @{term \<open>\<h>\<s>\<close>} of the homomorphism that works on mixed words, since we will encounter these during our induction step.
