...
definition trans1Tm :: "'n::infinite \<Rightarrow> 't \<Rightarrow> ('n, 't) cfg \<Rightarrow> ('n, 't) cfg \<Rightarrow> bool" where 
      "trans1Tm A t G G' \<equiv> (
...
definition prodTms :: "('n,'t) prod \<Rightarrow> nat" where
  "prodTms p \<equiv> (if length (snd p) \<le> 1 then 0 else length (filter (isTmnlSym) (snd p)))"

definition prodNts :: "('n,'t) prod \<Rightarrow> nat" where
  "prodNts p \<equiv> (if length (snd p) \<le> 2 then 0 else length (filter (isNonTmnlSym) (snd p)))"
...
proof 
  ...
  proof 
    ...
    hence "length (snd p) \<le> 1 \<or> (\<nexists>t. Tm t \<in> set (snd p))"
      by auto
    hence "length (snd p) \<le> 1 \<or> length (filter (isTmnlSym) (snd p)) = 0"
      unfolding isTmnlSym_def by (auto simp: filter_empty_conv)
    thus "prodTms p = 0"
...
definition cnf :: "('n,'t) cfg \<Rightarrow> bool" where
  "cnf G = ((badTmsCount (prods G) = 0) \<and> (badNtsCount (prods G) = 0))"

lemma cnf_bin_un: "(binary G \<and> uniform G) \<longleftrightarrow> cnf G"
  unfolding cnf_def using binary_badNtsCount uniform_badTmsCount sorry

definition trans2Nt :: "'n::infinite \<Rightarrow> 'n \<Rightarrow> 'n \<Rightarrow> ('n,'t) cfg \<Rightarrow> ('n,'t) cfg \<Rightarrow> bool" where
      "trans2Nt A B\<^sub>1 B\<^sub>2 G G' \<equiv> (
...
lemma trans2Nt_noeProds:
  assumes "noeProds (set (prods G))"
    and "trans2Nt A B\<^sub>1 B\<^sub>2 G G'"
  shows "noeProds (set (prods G'))"
...
proof -
  ...
  proof (cases "u = A")
    ...
    proof (cases "v = [Nt B\<^sub>1,Nt B\<^sub>2]")
      ...
      hence "elim (A, [Nt B\<^sub>1,Nt B\<^sub>2]) rhs = elim (A, [Nt B\<^sub>1,Nt B\<^sub>2]) p' @ [Nt B\<^sub>1,Nt B\<^sub>2] @ elim (A, [Nt B\<^sub>1,Nt B\<^sub>2]) s'"
        using assms(1) trans2Nt_aux1[of A B\<^sub>1 B\<^sub>2 G G'] by fastforce 
      then show ?thesis
        using 1 by simp
    next
      case False
  ...
  next
    ...
    proof (cases "(Nt A) \<in> set v")
      ...
      hence 1: "v = p@[Nt A]@s \<and> Nt A \<notin> set p \<and> Nt A \<notin> set s" 
        using True \<open>?lrps\<close> \<open>?uv\<close> assms slemma4_4Nt[of A B\<^sub>1 B\<^sub>2 G G'] trans2Nt_aux1[of A B\<^sub>1 B\<^sub>2 G G'] by auto
...
proof 
  ...
  moreover have "elim (A, [Tm t]) [Nt (start G)] = [Nt (start G)]"
    using assms fresh unfolding trans1Tm_def 
...
proof
  ...
  moreover have "elim (A, [Nt B\<^sub>1, Nt B\<^sub>2]) [Nt (start G)] = [Nt (start G)]"
    using assms fresh unfolding trans2Nt_def 
...
proof 
  ...
  assume "w \<in> L G"
  hence "set (prods G) \<turnstile> [Nt (start G')] \<Rightarrow>* map Tm w"
    using assms unfolding Lang_def trans1Tm_def by auto 
  thus "w \<in> L G'" 
...
proof 
  ...
  assume "w \<in> L G"
  hence "set (prods G) \<turnstile> [Nt (start G')] \<Rightarrow>* map Tm w"
    using assms unfolding Lang_def trans2Nt_def by auto 
  thus "w \<in> L G'" 
...
lemma trans1TmRtc_noeProds: 
  assumes "((\<lambda>x y. \<exists>A t. trans1Tm A t x y) ^**) G G'"
    and "noeProds (set (prods G))"
  shows "noeProds (set (prods G'))"
  using assms by (induction) (auto simp: trans1Tm_noeProds)

lemma trans2NtRtc_noeProds:
  assumes "((\<lambda>x y. \<exists>A t B\<^sub>1 B\<^sub>2. trans2Nt A B\<^sub>1 B\<^sub>2 x y) ^**) G G'"
    and "noeProds (set (prods G))"
  shows "noeProds (set (prods G'))"
...
lemma trans1Tm_Nts: 
  assumes "trans1Tm A t G G'" "S \<in> Nts (set (prods G))"
...
lemma trans1TmRtc_Nts: 
  assumes "((\<lambda>x y. \<exists>A t. trans1Tm A t x y) ^**) G G'" "S \<in> Nts (set (prods G))"
  shows "S \<in> Nts (set (prods G'))"
  using assms by (induction rule: rtranclp.induct) (auto simp: trans1Tm_Nts)

lemma trans2Nt_Nts: 
  assumes "trans2Nt A B\<^sub>1 B\<^sub>2 G G'" "S \<in> Nts (set (prods G))"
  shows "S \<in> Nts (set (prods G'))"
proof -
  obtain l r p s where "(l,r) \<in> set (prods G) \<and> (r = p@[Nt B\<^sub>1,Nt B\<^sub>2]@s) \<and> (p \<noteq> [] \<or> s \<noteq> []) \<and> (A \<notin> Nts (set (prods G)))
    \<and> (set (prods G') = ((set (prods G) - {(l,r)}) \<union> {(A, [Nt B\<^sub>1,Nt B\<^sub>2]), (l, p@[Nt A]@s)}))" (is "?lrps")
...
lemma trans2NtRtc_Nts: 
  assumes "((\<lambda>x y. \<exists>A t B\<^sub>1 B\<^sub>2. trans2Nt A B\<^sub>1 B\<^sub>2 x y) ^**) G G'" "S \<in> Nts (set (prods G))"
  shows "S \<in> Nts (set (prods G'))"
...
theorem cnf_lemma: 
  assumes "((\<lambda>x y. \<exists>A t. trans1Tm A t x y) ^**) G G'"
    and "((\<lambda>x y. \<exists>A B\<^sub>1 B\<^sub>2. trans2Nt A B\<^sub>1 B\<^sub>2 x y) ^**) G' G''"
  shows "L G = L G''"
...
lemma lemma7_a: "trans1Tm A t G G' \<Longrightarrow> badTmsCount (prods G) > 0"
  using lemma6_a by fastforce

lemma lemma7_b: "trans2Nt A B\<^sub>1 B\<^sub>2 G G' \<Longrightarrow> badNtsCount (prods G) > 0"
...
lemma lemma15_a:
  assumes "((\<lambda>x y. \<exists>A B\<^sub>1 B\<^sub>2. trans2Nt A B\<^sub>1 B\<^sub>2 x y) ^**) G G'"
    and "badTmsCount (prods G) = 0"
  shows "badTmsCount (prods G') = 0"
...
lemma lemma8_a:
  assumes "badTmsCount (prods G) > 0"
  shows "\<exists>G' A t. trans1Tm A t G G'"
proof -
  ...
  from this obtain A G' where "A = fresh G \<and> G' = cfg (removeAll (l, r) (prods G) @ [(A, [Tm t]), (l, p @ [Nt A] @ s)]) (start G)" 
    by auto
  hence "trans1Tm A t G G'"
    unfolding trans1Tm_def using \<open>?lr\<close>  \<open>?ps\<close> by auto
...
lemma lemma8_b:
  ...
  shows "\<exists>G' A B\<^sub>1 B\<^sub>2. trans2Nt A B\<^sub>1 B\<^sub>2 G G'"
proof -
  ...
  from this obtain B\<^sub>1 B\<^sub>2 where "X = Nt B\<^sub>1 \<and> Y = Nt B\<^sub>2"
    using isNonTmnlSym_def \<open>?psXY\<close> by fastforce
  ...
  from this obtain A G' where "A = fresh G \<and> G' = cfg (removeAll (l, r) (prods G) @ [(A, [Nt B\<^sub>1, Nt B\<^sub>2]), (l, p @ [Nt A] @ s)]) (start G)"
    by simp
  hence "trans2Nt A B\<^sub>1 B\<^sub>2 G G'"
    unfolding trans2Nt_def using \<open>?lr\<close> \<open>?B\<close> by auto
  thus ?thesis by blast
qed

lemma lemma11_a: "\<nexists>G' A t. trans1Tm A t G G' \<Longrightarrow> badTmsCount (prods G) = 0"
  using lemma8_a by blast

lemma lemma11_b: 
  assumes "\<nexists>G' A B\<^sub>1 B\<^sub>2. trans2Nt A B\<^sub>1 B\<^sub>2 G G'"
    and "badTmsCount (prods G) = 0" 
    shows "badNtsCount (prods G) = 0"
  using assms lemma8_b by blast

lemma trans1Tm_2: "\<exists>G'. ((\<lambda>x y. \<exists>A t. trans1Tm A t x y) ^**) G G' \<and> (badTmsCount (prods G') = 0)"
proof (induction "badTmsCount (prods G)" arbitrary: G rule: less_induct)
  case less
  then show ?case
  proof (cases "badTmsCount (prods G) = 0")
    case False
    from this obtain G' A t where "trans1Tm A t G G'" (is "?G'")
      using lemma8_a by blast
    hence "badTmsCount (prods G') < badTmsCount (prods G)"
      using lemma6_a[of A t G G'] by blast
    from this obtain G'' where "(\<lambda>x y. \<exists>A t. trans1Tm A t x y)\<^sup>*\<^sup>* G' G'' \<and> badTmsCount (prods G'') = 0"
      using less by blast
    thus ?thesis 
      using \<open>?G'\<close> converse_rtranclp_into_rtranclp[of "(\<lambda>x y. \<exists>A t. trans1Tm A t x y)" G G' G''] by blast
  qed blast
qed

lemma trans2Nt_2: 
  assumes "badTmsCount (prods G) = 0"
    shows "\<exists>G'. ((\<lambda>x y. \<exists>A B\<^sub>1 B\<^sub>2. trans2Nt A B\<^sub>1 B\<^sub>2 x y) ^**) G G' \<and> (badNtsCount (prods G') = 0)"
using assms proof (induction "badNtsCount (prods G)" arbitrary: G rule: less_induct)
  case less
  then show ?case 
  proof (cases "badNtsCount (prods G) = 0")
    case False
    from this obtain G' A B\<^sub>1 B\<^sub>2 where "trans2Nt A B\<^sub>1 B\<^sub>2 G G'" (is "?G'")
      using assms lemma8_b less(2) by blast
    hence "badNtsCount (prods G') < badNtsCount (prods G)"
      using lemma6_b by blast
    from this obtain G'' where "(\<lambda>x y. \<exists>A B\<^sub>1 B\<^sub>2. trans2Nt A B\<^sub>1 B\<^sub>2 x y)\<^sup>*\<^sup>* G' G'' \<and> badNtsCount (prods G'') = 0"
      using less slemma15_a[of A B\<^sub>1 B\<^sub>2 G G'] \<open>?G'\<close> by blast
    then show ?thesis 
      using \<open>?G'\<close> converse_rtranclp_into_rtranclp[of "(\<lambda>x y. \<exists>A B\<^sub>1 B\<^sub>2. trans2Nt A B\<^sub>1 B\<^sub>2 x y)" G G' G''] by blast
  qed blast
qed

theorem thm4_5: "\<forall>G::('n::infinite,'t) cfg. \<exists>G'::('n,'t)cfg. cnf G' \<and> L G = L G'"
proof 
  fix G::"('n::infinite,'t) cfg"
  obtain G' where "((\<lambda>x y. \<exists>A t. trans1Tm A t x y) ^**) G G' \<and> (badTmsCount (prods G') = 0)" (is "?G'")
    using trans1Tm_2 by blast
  obtain G'' where "((\<lambda>x y. \<exists>A B\<^sub>1 B\<^sub>2. trans2Nt A B\<^sub>1 B\<^sub>2 x y) ^**) G' G'' \<and> (badNtsCount (prods G'') = 0) \<and> (badTmsCount (prods G'') = 0)" (is "?G''")
    using \<open>?G'\<close> trans2Nt_2 lemma15_a by blast
  hence "cnf G''"
    using cnf_def by blast
  moreover have "L G = L G''"
    using \<open>?G'\<close> \<open>?G''\<close> cnf_lemma by blast
  ultimately show "\<exists>G'::('n,'t)cfg. cnf G' \<and> L G = L G'"
    by blast
qed

theorem cnf_noe_noUnit: 
  assumes "noeProds (set (prods (G::('n::infinite,'t) cfg)))" "noUnitProds (set (prods G))"
  shows "\<exists>G'::('n,'t) cfg. (cnf G') \<and> (L G = L G') \<and> noeProds (set (prods G')) \<and> noUnitProds (set (prods G'))"
proof -
  obtain G' where "((\<lambda>x y. \<exists>A t. trans1Tm A t x y) ^**) G G' \<and> (badTmsCount (prods G') = 0) \<and> noeProds (set (prods G')) \<and> noUnitProds (set (prods G'))" (is "?G'")
    using assms trans1Tm_2 trans1TmRtc_noeProds trans1TmRtc_noUnitProds by blast
  obtain G'' where "((\<lambda>x y. \<exists>A B\<^sub>1 B\<^sub>2. trans2Nt A B\<^sub>1 B\<^sub>2 x y) ^**) G' G'' \<and> (badNtsCount (prods G'') = 0) \<and> (badTmsCount (prods G'') = 0)" (is "?G''")
    using \<open>?G'\<close> trans2Nt_2 lemma15_a by blast
  hence "cnf G'' \<and> noeProds (set (prods G'')) \<and> noUnitProds (set (prods G''))"
    using \<open>?G'\<close> cnf_def trans2NtRtc_noeProds trans2NtRtc_noUnitProds by fastforce
  moreover have "L G = L G''"
    using \<open>?G'\<close> \<open>?G''\<close> cnf_lemma by blast
  ultimately show ?thesis
...
definition isCnf :: "('n, 't) cfg \<Rightarrow> bool" where
  "isCnf G \<equiv> (\<forall>(l,r) \<in> set (prods G).
    (length r = 2 \<and> list_all (isNonTmnlSym) r) \<or> 
    (length r = 1 \<and> list_all (isTmnlSym) r))"

lemma isCnf_eq_cnf: "isCnf G \<longleftrightarrow> (cnf G \<and> noeProds (set (prods G)) \<and> noUnitProds (set (prods G)))"
proof 
  assume "isCnf G" (is "?assm")
  hence "noeProds (set (prods G))"
    unfolding isCnf_def noeProds_def by fastforce
  moreover have "noUnitProds (set (prods G))"
    using \<open>?assm\<close> unfolding isCnf_def noUnitProds_def isNonTmnlSym_def isTmnlSym_def by fastforce
  moreover have "cnf G"
    using \<open>?assm\<close> unfolding isCnf_def cnf_def isNonTmnlSym_def isTmnlSym_def prodNts_def prodTms_def sorry
  ultimately show "cnf G \<and> noeProds (set (prods G)) \<and> noUnitProds (set (prods G))"
    by blast
next 
  assume "cnf G \<and> noeProds (set (prods G)) \<and> noUnitProds (set (prods G))" (is "?assm")
  hence "binary G \<and> uniform G"
    using cnf_bin_un by blast
  hence "\<forall>(A, \<alpha>) \<in> set (prods G). length \<alpha> \<le> 2"
    unfolding binary_def by simp
  hence "\<forall>(A, \<alpha>) \<in> set (prods G). length \<alpha> = 2 \<or> length \<alpha> = 1"
    using \<open>?assm\<close> nat_neq_iff unfolding noeProds_def by fastforce
  thus "isCnf G"
    unfolding noUnitProds_def isCnf_def sorry
qed

theorem cnf_exists: 
  shows "\<forall>G::('n::infinite,'t) cfg. \<exists>G'::('n,'t) cfg. (isCnf G') \<and> (L G' = L G - {[]})"
proof
  fix G::"('n,'t)cfg"
  ...
  obtain G\<^sub>u where "G\<^sub>u = cfg P\<^sub>u (start G)" (is "?G\<^sub>u")
    by simp
  hence 1: "noeProds (set (prods G\<^sub>u)) \<and> noUnitProds (set (prods G\<^sub>u))"
    using \<open>?P\<^sub>0\<close> \<open>?P\<^sub>u\<close> negrImpnoeProds upgrImpnoUnitProds upgr_noeProds by fastforce
  ...
  obtain G'::"('n,'t) cfg" where "cnf G' \<and> L G\<^sub>u = L G' \<and> noeProds (set (prods G')) \<and> noUnitProds (set (prods G'))" (is "?G'")
    using 1 cnf_noe_noUnit by blast
  hence "isCnf G'" 
    using \<open>?G'\<close> isCnf_eq_cnf by blast
  moreover have "L G' = L G - {[]}"
    using 2 \<open>?G'\<close> by blast
  ultimately show "\<exists>G'::('n,'t) cfg. (isCnf G') \<and> (L G' = L G - {[]})" by blast
qed
