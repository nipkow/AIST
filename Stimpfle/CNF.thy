...
(* Termination *)
theorem cnf_lemma2: "\<forall>G G'. ((\<lambda>x y. \<exists>A t. trans1Tmnl A t x y) ^**) G G' \<Longrightarrow> (L G = L G')"
  sorry

theorem trans1Tmnl_2: 
  assumes "infinite(UNIV::'a set)"
  shows "\<exists>G'. ((\<lambda>x y. \<exists>A t. trans1Tmnl A t x y) ^**) G G' \<and> (badTmnlsCount G' = 0)"
proof (induction "badTmnlsCount G")
  case 0
...
definition isCnf :: "('n, 't) prods \<Rightarrow> bool" where
      "isCnf P \<equiv> (\<forall>l r. (l,r) \<in> set P \<longrightarrow> (
    (length r = 2 \<and> list_all (isNonTmnlSym) r) \<or> 
    (length r = 1 \<and> list_all (isTmnlSym) r)))"


theorem cnf_trans: "\<forall>G. infinite(UNIV::'a set) \<and> [] \<notin> L G \<Longrightarrow> \<exists>G'. isCnf G'" 
  sorry
end