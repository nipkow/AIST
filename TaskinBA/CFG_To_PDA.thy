...
fun cfg_to_pda_trans_fun :: "sing_st \<Rightarrow> 't \<Rightarrow> ('n,'t) sym \<Rightarrow> (sing_st \<times> ('n,'t) syms) set" where
  "cfg_to_pda_trans_fun Q_loop a (Tm b) = (if a = b then {(Q_loop, [])} else {})"
| "cfg_to_pda_trans_fun _ _ _ = {}"

fun cfg_to_pda_eps_fun :: "sing_st \<Rightarrow> ('n,'t) sym \<Rightarrow> (sing_st \<times> ('n,'t) syms) set" where
  "cfg_to_pda_eps_fun Q_loop (Nt A) = {(Q_loop, \<alpha>)| \<alpha>. (A, \<alpha>) \<in> Prods G}"
...
proof -
  from assms have "(\<exists>\<beta>\<^sub>0. w' = w \<and> \<beta> = \<beta>\<^sub>0@\<gamma> \<and> (Q_loop, \<beta>\<^sub>0) \<in> cfg_to_pda_eps_fun Q_loop (Nt A)) 
                     \<or> (\<exists>a \<beta>\<^sub>0. w = a # w' \<and> \<beta> = \<beta>\<^sub>0@\<gamma> \<and> (Q_loop,\<beta>\<^sub>0) \<in> cfg_to_pda_trans_fun Q_loop a (Nt A))"
    using pda.step\<^sub>1_rule[OF pda_cfg_to_pda] by (simp add: cfg_to_pda_pda_def)
  thus ?thesis
    by (induction Q_loop "Nt A :: ('n, 't) sym" rule: cfg_to_pda_eps_fun.induct) auto
qed
...
proof -
  from assms have "(\<exists>\<beta>\<^sub>0. w' = w \<and> \<beta>' = \<beta>\<^sub>0@\<beta> \<and> (Q_loop, \<beta>\<^sub>0) \<in> cfg_to_pda_eps_fun Q_loop (Tm a)) 
                   \<or> (\<exists>a\<^sub>0 \<beta>\<^sub>0. w = a\<^sub>0#w' \<and> \<beta>' = \<beta>\<^sub>0@\<beta> \<and> (Q_loop, \<beta>\<^sub>0) \<in> cfg_to_pda_trans_fun Q_loop a\<^sub>0 (Tm a))"
    using pda.step\<^sub>1_rule[OF pda_cfg_to_pda] by (simp add: cfg_to_pda_pda_def)
  thus ?thesis
    by (induction Q_loop "Tm a :: ('n, 't) sym" rule: cfg_to_pda_eps_fun.induct, auto) (metis emptyE, metis empty_iff prod.inject singletonD)
qed
