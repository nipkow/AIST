...
fun stack_of_final :: "'q st_extended \<Rightarrow> 'a \<Rightarrow> 's sym_extended \<Rightarrow> ('q st_extended \<times> 's sym_extended list) set" where
  "stack_of_final (Old_st q) a (Old_sym Z) = (\<lambda>(p, \<alpha>). (Old_st p, map Old_sym \<alpha>)) ` (trans_fun M q a Z)"
| "stack_of_final  _ _ _ = {}"

fun stack_of_final_eps :: "'q st_extended \<Rightarrow> 's sym_extended \<Rightarrow> ('q st_extended \<times> 's sym_extended list) set" where
  "stack_of_final_eps (Old_st q) (Old_sym Z) = (if q \<in> final_states M then {(New_final, [Old_sym Z])} else {}) \<union> 
                                                        (\<lambda>(p, \<alpha>). (Old_st p, map Old_sym \<alpha>)) ` (eps_fun M q Z)"
| "stack_of_final_eps (Old_st q) New_sym = (if q \<in> final_states M then {(New_final, [New_sym])} else {})"
| "stack_of_final_eps New_init New_sym = {(Old_st (init_state M), [Old_sym (init_symbol M), New_sym])}"
| "stack_of_final_eps New_final _ = {(New_final, [])}"
| "stack_of_final_eps _ _ = {}"
...
definition stack_of_final_pda :: "('q st_extended, 'a, 's sym_extended) pda" where
  "stack_of_final_pda \<equiv> \<lparr> init_state = New_init, init_symbol = New_sym, final_states = {New_final}, 
                    trans_fun = stack_of_final, eps_fun = stack_of_final_eps\<rparr>"

lemma pda_final_to_stack:
  "pda stack_of_final_pda"
proof (standard, goal_cases)
  case (1 p x z)
  have "finite (stack_of_final p x z)"
    by (induction p x z rule: stack_of_final.induct) (auto simp: finite_trans)
  then show ?case
    by (simp add: stack_of_final_pda_def)
next
  case (2 p z)
  have "finite (stack_of_final_eps p z)"
    by (induction p z rule: stack_of_final_eps.induct) (auto simp: finite_eps)
  then show ?case
    by (simp add: stack_of_final_pda_def)
qed
...
proof
  ...
  from rule have "(\<exists>\<beta>. w\<^sub>2 = w\<^sub>1 \<and> map Old_sym \<alpha>\<^sub>2 = map Old_sym \<beta> @ map Old_sym \<alpha> \<and> 
                    (Old_st p\<^sub>2, map Old_sym \<beta>) \<in> eps_fun stack_of_final_pda (Old_st p\<^sub>1) (Old_sym Z)) 
                \<or> (\<exists>a \<beta>. w\<^sub>1 = a # w\<^sub>2 \<and> map Old_sym \<alpha>\<^sub>2 = map Old_sym \<beta> @ map Old_sym \<alpha> \<and> 
                    (Old_st p\<^sub>2, map Old_sym \<beta>) \<in> trans_fun stack_of_final_pda (Old_st p\<^sub>1) a (Old_sym Z))"
    using stack_of_final_pda_trans stack_of_final_pda_eps by fastforce
  hence "(\<exists>\<beta>. w\<^sub>2 = w\<^sub>1 \<and> map Old_sym \<alpha>\<^sub>2 = \<beta> @ map Old_sym \<alpha> \<and> (Old_st p\<^sub>2, \<beta>) \<in> eps_fun stack_of_final_pda (Old_st p\<^sub>1) (Old_sym Z)) 
            \<or> (\<exists>a \<beta>. w\<^sub>1 = a # w\<^sub>2 \<and> map Old_sym \<alpha>\<^sub>2 = \<beta> @ map Old_sym \<alpha> \<and> (Old_st p\<^sub>2, \<beta>) \<in> trans_fun stack_of_final_pda (Old_st p\<^sub>1) a (Old_sym Z))"  by blast
...
next
  ...
  then obtain Z \<alpha> where map_\<alpha>\<^sub>1_def: "map Old_sym \<alpha>\<^sub>1 = Old_sym Z # map Old_sym \<alpha>" and rule: 
        "(\<exists>\<beta>. w\<^sub>2 = w\<^sub>1 \<and> map Old_sym \<alpha>\<^sub>2 = \<beta>@ map Old_sym \<alpha> \<and> 
            (Old_st p\<^sub>2, \<beta>) \<in> eps_fun stack_of_final_pda (Old_st p\<^sub>1) (Old_sym Z)) 
       \<or> (\<exists>a \<beta>. w\<^sub>1 = a # w\<^sub>2 \<and> map Old_sym \<alpha>\<^sub>2 = \<beta>@ map Old_sym \<alpha> \<and> 
            (Old_st p\<^sub>2,\<beta>) \<in> trans_fun stack_of_final_pda (Old_st p\<^sub>1) a (Old_sym Z))"
...
proof -
  from assms obtain Z \<alpha> where \<alpha>\<^sub>1_with_new_def: "\<alpha>_with_new \<alpha>\<^sub>1 = Z # \<alpha>" and rule: 
      "(\<exists>\<beta>. w\<^sub>2 = w\<^sub>1 \<and> \<alpha>_with_new \<alpha>\<^sub>2 = \<beta>@\<alpha> \<and> (Old_st p\<^sub>2, \<beta>) \<in> stack_of_final_eps (Old_st p\<^sub>1) Z) 
          \<or> (\<exists>a \<beta>. w\<^sub>1 = a # w\<^sub>2 \<and> \<alpha>_with_new \<alpha>\<^sub>2 = \<beta>@\<alpha> \<and> (Old_st p\<^sub>2,\<beta>) \<in> stack_of_final (Old_st p\<^sub>1) a Z)"
    using pda.step\<^sub>1_rule_ext[OF pda_final_to_stack] by (auto simp: stack_of_final_pda_def)
  from rule have "Z \<noteq> New_sym"
    by (induction "Old_st p\<^sub>1" Z rule: stack_of_final_eps.induct) (auto, metis empty_iff fst_conv singletonD st_extended.simps(5))
...
proof -
  from assms obtain Z \<alpha> where rule: 
        "(\<exists>\<beta>. w\<^sub>2 = w\<^sub>1 \<and> \<alpha>\<^sub>2 = \<beta>@\<alpha> \<and> (p\<^sub>2, \<beta>) \<in> stack_of_final_eps (Old_st p\<^sub>1) Z) 
            \<or> (\<exists>a \<beta>. w\<^sub>1 = a # w\<^sub>2 \<and> \<alpha>\<^sub>2 = \<beta>@\<alpha> \<and> (p\<^sub>2,\<beta>) \<in> stack_of_final (Old_st p\<^sub>1) a Z)"
    using pda.step\<^sub>1_rule_ext[OF pda_final_to_stack] by (auto simp: stack_of_final_pda_def)+
  thus ?thesis 
    by (induction "Old_st p\<^sub>1" Z rule: stack_of_final_eps.induct, auto) (metis empty_iff fst_conv singletonD)+
qed
...
proof -
  from assms obtain Z \<alpha> where \<alpha>\<^sub>1_def: "\<alpha>\<^sub>1 = Z#\<alpha>" and rule: 
        "(\<exists>\<beta>. w\<^sub>2 = w\<^sub>1 \<and> \<alpha>\<^sub>2 = \<beta>@\<alpha> \<and> (p\<^sub>2,\<beta>) \<in> stack_of_final_eps New_final Z) 
          \<or> (\<exists>a \<beta>. w\<^sub>1 = a # w\<^sub>2 \<and> \<alpha>\<^sub>2 = \<beta>@\<alpha> \<and> (p\<^sub>2,\<beta>) \<in> stack_of_final New_final a Z)"
    using pda.step\<^sub>1_rule_ext[OF pda_final_to_stack] by (auto simp: stack_of_final_pda_def)
  thus ?thesis 
    by (induction "New_final:: 'q st_extended" Z rule: stack_of_final_eps.induct) auto
qed
...
proof (induction "(Old_st p\<^sub>1, w\<^sub>1, \<alpha>_with_new \<alpha>\<^sub>1)" "(Old_st p\<^sub>2, w\<^sub>2, \<gamma>)" arbitrary: p\<^sub>2 w\<^sub>2 \<gamma> rule: pda.steps_induct2_bw[OF pda_final_to_stack])
  ...
  from p\<^sub>2_def 3(2) obtain Z \<alpha> where \<alpha>\<^sub>2_split: "\<alpha>\<^sub>2 = Z # \<alpha>" and rule:
    "(\<exists>\<beta>. w\<^sub>3 = w\<^sub>2 \<and> \<alpha>\<^sub>3 = \<beta> @ \<alpha> \<and> (Old_st p\<^sub>3, \<beta>) \<in> stack_of_final_eps (Old_st p\<^sub>2') Z) 
     \<or> (\<exists>a \<beta>. w\<^sub>2 = a # w\<^sub>3 \<and> \<alpha>\<^sub>3 = \<beta> @ \<alpha> \<and> (Old_st p\<^sub>3, \<beta>) \<in> stack_of_final (Old_st p\<^sub>2') a Z)"
    using pda.step\<^sub>1_rule_ext[OF pda_final_to_stack] by (auto simp: stack_of_final_pda_def)
  hence "\<exists>Z'. Z = Old_sym Z'"
    by (induction "Old_st p\<^sub>2'" Z rule: stack_of_final_eps.induct) 
  ...
  with rule show ?case
    by (induction "Old_st p\<^sub>2'" Z rule: stack_of_final_eps.induct, auto)
...
proof (induction \<gamma>)
  case (Cons Z \<gamma>)
  have "(New_final, []) \<in> stack_of_final_eps New_final Z" by simp
...
using assms proof (induction "Suc n" "(Old_st p\<^sub>1, w\<^sub>1, \<alpha>\<^sub>1)" "(New_final :: 'q st_extended, w\<^sub>2, \<alpha>\<^sub>2)" 
  ...
  then show ?case proof (cases n)
    ...
    from p\<^sub>2_def 3(3) obtain Z \<alpha> where \<alpha>\<^sub>2_def: "\<alpha>\<^sub>2 = Z # \<alpha>" and rule: 
            "(\<exists>\<beta>. w\<^sub>3 = w\<^sub>2 \<and> \<alpha>\<^sub>3 = \<beta>@\<alpha> \<and> (New_final,\<beta>) \<in> stack_of_final_eps (Old_st p\<^sub>1) Z) 
               \<or> (\<exists>a \<beta>. w\<^sub>2 = a # w\<^sub>3 \<and> \<alpha>\<^sub>3 = \<beta>@\<alpha> \<and> (New_final,\<beta>) \<in> stack_of_final (Old_st p\<^sub>1) a Z)"
      using pda.step\<^sub>1_rule_ext[OF pda_final_to_stack] by (auto simp: stack_of_final_pda_def)
    from \<alpha>\<^sub>2_def rule have p\<^sub>1_final: "p\<^sub>1 \<in> final_states M" and w23: "w\<^sub>3 = w\<^sub>2" and a23: "\<alpha>\<^sub>3 = \<alpha>\<^sub>2"
      by (induction "Old_st p\<^sub>1" Z rule: stack_of_final_eps.induct, auto)
  ...
  next
    ...
    then show ?thesis proof (cases "p\<^sub>2 = New_final")
      ...
      from True 3(3) obtain Z' \<alpha>' where "\<alpha>\<^sub>2 = Z' # \<alpha>'" and rule: 
          "(\<exists>\<beta>. w\<^sub>3 = w\<^sub>2 \<and> \<alpha>\<^sub>3 = \<beta>@\<alpha>' \<and> (New_final,\<beta>) \<in> stack_of_final_eps New_final Z') 
             \<or> (\<exists>a \<beta>. w\<^sub>2 = a # w\<^sub>3 \<and> \<alpha>\<^sub>3 = \<beta>@\<alpha>' \<and> (New_final,\<beta>) \<in> stack_of_final New_final a Z')"
        using pda.step\<^sub>1_rule_ext[OF pda_final_to_stack] by (auto simp: stack_of_final_pda_def)
    ...
    next
      ...
      from p\<^sub>2_def 3(3) obtain Z' \<alpha>' where "\<alpha>\<^sub>2 = Z' # \<alpha>'" and 
            "(\<exists>\<beta>. w\<^sub>3 = w\<^sub>2 \<and> \<alpha>\<^sub>3 = \<beta>@\<alpha>' \<and> (New_final,\<beta>) \<in> stack_of_final_eps (Old_st p\<^sub>2') Z') 
               \<or> (\<exists>a \<beta>. w\<^sub>2 = a # w\<^sub>3 \<and> \<alpha>\<^sub>3 = \<beta>@\<alpha>' \<and> (New_final,\<beta>) \<in> stack_of_final (Old_st p\<^sub>2') a Z')"
        using pda.step\<^sub>1_rule_ext[OF pda_final_to_stack] by (auto simp: stack_of_final_pda_def)
...
proof
  ...
  from q_final have "(New_final, [Z]) \<in> stack_of_final_eps (Old_st q) Z"
    by (induction "Old_st q" Z rule: stack_of_final_eps.induct) auto
