...
definition final_to_stack_pda :: "('q st_extended, 'a, 's sym_extended) pda" where
  "final_to_stack_pda \<equiv> \<lparr> states = stack_final_states, init_state = New_init, init_symbol = New_sym, final_states = {New_final}, 
...
proof (standard, goal_cases)
  case 1
  then show ?case
    unfolding final_to_stack_pda_def stack_final_states_def by simp
next
  case 2
  then show ?case
    unfolding final_to_stack_pda_def stack_final_states_def by simp
next
  case (3 p x z)
  from 3[unfolded final_to_stack_pda_def stack_final_states_def] 
  have "fst ` final_to_stack_trans_fun p x z \<subseteq> Old_st ` states M \<union> {New_init, New_final}"
    using trans by (induction p x z rule: final_to_stack_trans_fun.induct) fastforce+
  then show ?case
    unfolding final_to_stack_pda_def stack_final_states_def by simp
next
  case (4 p z)
  from 4[unfolded final_to_stack_pda_def stack_final_states_def] 
  have "fst ` final_to_stack_eps_fun p z \<subseteq> Old_st ` states M \<union> {New_init, New_final}"
    using eps init by (induction p z rule: final_to_stack_eps_fun.induct) (auto, fastforce)
  then show ?case
    unfolding final_to_stack_pda_def stack_final_states_def by simp
next
  case 5
  then show ?case
    unfolding final_to_stack_pda_def stack_final_states_def using finite_states by simp
next
  case (6 p x z)
  have "finite (final_to_stack_trans_fun p x z)"
    by (induction p x z rule: final_to_stack_trans_fun.induct) (auto simp: finite_trans)
  then show ?case
    unfolding final_to_stack_pda_def by simp
next
  case (7 p z)
...
proof
  ...
  thus "(Old_st p, map Old_sym \<beta>) \<in> trans_fun final_to_stack_pda (Old_st q) a (Old_sym Z)"
    unfolding final_to_stack_pda_def by auto
next
  assume "(Old_st p, map Old_sym \<beta>) \<in> trans_fun final_to_stack_pda (Old_st q) a (Old_sym Z)"
...
proof
  ...
  thus "(Old_st p, map Old_sym \<beta>) \<in> eps_fun final_to_stack_pda (Old_st q) (Old_sym Z)"
    unfolding final_to_stack_pda_def by auto
next
  assume "(Old_st p, map Old_sym \<beta>) \<in> eps_fun final_to_stack_pda (Old_st q) (Old_sym Z)"
  hence "(Old_st p, map Old_sym \<beta>) \<in> (\<lambda>(p, \<alpha>). (Old_st p, map Old_sym \<alpha>)) ` (eps_fun M q Z)"
    unfolding final_to_stack_pda_def by auto (metis empty_iff fst_conv singletonD st_extended.simps(5))
...
lemma final_to_stack_pda_step:
  "pda.step\<^sub>1 M (p\<^sub>1, w\<^sub>1, \<alpha>\<^sub>1) (p\<^sub>2, w\<^sub>2, \<alpha>\<^sub>2) \<longleftrightarrow>
             pda.step\<^sub>1 final_to_stack_pda (Old_st p\<^sub>1, w\<^sub>1, map Old_sym \<alpha>\<^sub>1) (Old_st p\<^sub>2, w\<^sub>2, map Old_sym \<alpha>\<^sub>2)"
proof
  ...
  with \<alpha>\<^sub>1_def show "pda.step\<^sub>1 final_to_stack_pda (Old_st p\<^sub>1, w\<^sub>1, map Old_sym \<alpha>\<^sub>1) (Old_st p\<^sub>2, w\<^sub>2, map Old_sym \<alpha>\<^sub>2)"
    using pda.step\<^sub>1_rule[OF pda_final_to_stack] by simp
next
  assume "pda.step\<^sub>1 final_to_stack_pda (Old_st p\<^sub>1, w\<^sub>1, map Old_sym \<alpha>\<^sub>1) (Old_st p\<^sub>2, w\<^sub>2, map Old_sym \<alpha>\<^sub>2)"
...
proof -
  from assms obtain Z' \<alpha>' where map_\<alpha>\<^sub>1_def: "\<alpha>_with_new \<alpha>\<^sub>1 = Z' # \<alpha>'" and rule: 
...
proof -
  from assms obtain Z' \<alpha>' where "\<alpha>\<^sub>1 = Z'#\<alpha>'" and rule: 
...
lemma final_to_stack_pda_from_final:
  assumes "pda.step\<^sub>1 final_to_stack_pda (New_final :: 'q st_extended, w\<^sub>1, \<alpha>\<^sub>1) (p\<^sub>2, w\<^sub>2, \<alpha>\<^sub>2)"
...
lemma final_to_stack_pda_from_finaln:
  assumes "pda.steps final_to_stack_pda (New_final :: 'q st_extended, w\<^sub>1, \<alpha>\<^sub>1) (p\<^sub>2, w\<^sub>2, \<alpha>\<^sub>2)"
  shows "\<exists>\<gamma>. p\<^sub>2 = New_final \<and> w\<^sub>2 = w\<^sub>1 \<and> \<alpha>\<^sub>1 = \<gamma>@\<alpha>\<^sub>2"
using assms proof (induction "(New_final :: 'q st_extended, w\<^sub>1, \<alpha>\<^sub>1)" "(p\<^sub>2, w\<^sub>2, \<alpha>\<^sub>2)" 
                     arbitrary: w\<^sub>1 \<alpha>\<^sub>1 rule: pda.steps_induct[OF pda_final_to_stack])
  case (3 w\<^sub>1 \<alpha>\<^sub>1 p' w' \<alpha>')
  from 3(1) obtain Z' where p'_final: "p' = New_final" and w_eq: "w' = w\<^sub>1" and \<alpha>\<^sub>1_def: "\<alpha>\<^sub>1 = Z'#\<alpha>'"
    using final_to_stack_pda_from_final by blast
  with 3(2,3) obtain \<gamma> where "p\<^sub>2 = New_final" and "w\<^sub>2 = w'" and "\<alpha>' = \<gamma> @ \<alpha>\<^sub>2" by blast
  with w_eq \<alpha>\<^sub>1_def show ?case by auto
...
lemma final_to_stack_pda_from_oldn:
  ...
  using assms proof (induction "(Old_st p\<^sub>1, w\<^sub>1, \<alpha>\<^sub>1)" "(p\<^sub>2, w\<^sub>2, \<alpha>\<^sub>2)" 
                      arbitrary: p\<^sub>1 w\<^sub>1 \<alpha>\<^sub>1 rule: pda.steps_induct[OF pda_final_to_stack])
  case (3 w\<^sub>1 \<alpha>\<^sub>1 p' w' \<alpha>')
  from 3(1) have case_dist: "(\<exists>p''. p' = Old_st p'') \<or> p' = New_final"
    using final_to_stack_pda_from_old by simp
  then show ?case proof (cases)
    assume "\<exists>p''. p' = Old_st p''"
    then obtain p'' where "p' = Old_st p''" by blast
    with 3(2,3) show ?case by auto
  next
    assume "\<not>(\<exists>p''. p' = Old_st p'')"
    with case_dist have "p' = New_final" by blast
    with 3(2) show ?case
      using final_to_stack_pda_from_finaln by blast
  qed
qed (auto simp: assms)
...
proof -
  from assms obtain n where stepn: "pda.stepn final_to_stack_pda n (Old_st p\<^sub>1, w\<^sub>1, \<alpha>_with_new \<alpha>\<^sub>1) (Old_st p\<^sub>2, w\<^sub>2, \<gamma>)"
  ...
  thus ?thesis proof 
   (induction n "(Old_st p\<^sub>1, w\<^sub>1, \<alpha>_with_new \<alpha>\<^sub>1)" "(Old_st p\<^sub>2, w\<^sub>2, \<gamma>)"
       arbitrary: p\<^sub>2 w\<^sub>2 \<gamma> rule: pda.stepn.induct[OF pda_final_to_stack])
    case (3 n p\<^sub>2 w\<^sub>2 \<alpha>\<^sub>2 w\<^sub>3 \<alpha>\<^sub>3 p\<^sub>3)
    from 3(1) have steps: "pda.steps final_to_stack_pda (Old_st p\<^sub>1, w\<^sub>1, \<alpha>_with_new \<alpha>\<^sub>1) (p\<^sub>2, w\<^sub>2, \<alpha>\<^sub>2)"
      using pda.stepn_steps[OF pda_final_to_stack] by blast
    obtain p\<^sub>2' where p\<^sub>2_def: "p\<^sub>2 = Old_st p\<^sub>2'"
      using final_to_stack_pda_from_oldn[OF steps] final_to_stack_pda_to_old[OF 3(3)] by blast
...
lemma final_to_stack_pda_stepn:
  "pda.stepn M n (p\<^sub>1, w\<^sub>1, \<alpha>\<^sub>1) (p\<^sub>2, w\<^sub>2, \<alpha>\<^sub>2) \<longleftrightarrow> 
      pda.stepn final_to_stack_pda n (Old_st p\<^sub>1, w\<^sub>1, \<alpha>_with_new \<alpha>\<^sub>1) (Old_st p\<^sub>2, w\<^sub>2, \<alpha>_with_new \<alpha>\<^sub>2)"
proof
  assume asm: "pda.stepn M n (p\<^sub>1, w\<^sub>1, \<alpha>\<^sub>1) (p\<^sub>2, w\<^sub>2, \<alpha>\<^sub>2)"
  thus "pda.stepn final_to_stack_pda n (Old_st p\<^sub>1, w\<^sub>1, \<alpha>_with_new \<alpha>\<^sub>1) (Old_st p\<^sub>2, w\<^sub>2, \<alpha>_with_new \<alpha>\<^sub>2)"
  proof (induction n "(p\<^sub>1, w\<^sub>1, \<alpha>\<^sub>1)" "(p\<^sub>2, w\<^sub>2, \<alpha>\<^sub>2)" arbitrary: p\<^sub>2 w\<^sub>2 \<alpha>\<^sub>2 rule: stepn.induct)
    case refl\<^sub>n
    then show ?case
      by (simp add: pda.refl\<^sub>n[OF pda_final_to_stack])
  next
  ...
  qed
next
  assume asm: "pda.stepn final_to_stack_pda n (Old_st p\<^sub>1, w\<^sub>1, \<alpha>_with_new \<alpha>\<^sub>1) (Old_st p\<^sub>2, w\<^sub>2, \<alpha>_with_new \<alpha>\<^sub>2)"
  ...
  next
    ...
    obtain p\<^sub>2' where p\<^sub>2_def: "p\<^sub>2 = Old_st p\<^sub>2'"
      using final_to_stack_pda_from_oldn[OF steps] final_to_stack_pda_to_old[OF 3(3)] by blast
    with steps obtain \<gamma> where \<alpha>\<^sub>2_def: "\<alpha>\<^sub>2 = \<alpha>_with_new \<gamma>"
...
proof (induction \<gamma>)
  case Nil
  then show ?case
    by (simp add: pda.steps_refl[OF pda_final_to_stack])
next
  case (Cons Z \<gamma>)
  ...
  with Cons.IH show ?case
    using pda.step\<^sub>1_stepn_one[OF pda_final_to_stack] pda.stepn_steps[OF pda_final_to_stack] 
          pda.steps_trans[OF pda_final_to_stack] by metis
qed
...
proof -
  from assms have "(\<exists>\<beta>. w\<^sub>2 = w\<^sub>1 \<and> \<alpha> = \<beta> \<and> (p\<^sub>2, \<beta>) \<in> final_to_stack_eps_fun New_init New_sym) 
                   \<or> (\<exists>a \<beta>. w\<^sub>1 = a # w\<^sub>2 \<and> \<alpha> = \<beta> \<and> (p\<^sub>2, \<beta>) \<in> final_to_stack_trans_fun New_init a New_sym)"
    using pda.step\<^sub>1_rule[OF pda_final_to_stack] by (simp add: final_to_stack_pda_def)
  thus ?thesis by simp
qed
...
lemma final_to_stack_pda_empty_only_final:
  assumes "pda.steps final_to_stack_pda (New_init, w\<^sub>1, [New_sym]) (q, w\<^sub>2, [])"
    shows "q = New_final"
proof -
  from assms obtain n where stepn: "pda.stepn final_to_stack_pda n (New_init, w\<^sub>1, [New_sym]) (q, w\<^sub>2, [])"
    using pda.stepn_steps[OF pda_final_to_stack] by blast
  then obtain n' where "n = Suc n'"
    using pda.stepn_not_refl[OF pda_final_to_stack] gr0_implies_Suc by blast
  with stepn have "pda.stepn final_to_stack_pda n'
  ...
  hence steps: "pda.steps final_to_stack_pda (Old_st (init_state M), w\<^sub>1, [Old_sym (init_symbol M), New_sym])
                                            (q, w\<^sub>2, [])"
    using pda.stepn_steps[OF pda_final_to_stack] by blast
  hence case_dist: "\<exists>q'. q = Old_st q' \<or> q = New_final"
    using final_to_stack_pda_from_oldn by blast
  with steps show ?thesis
    using final_to_stack_pda_bottom_elem[of "init_state M" w\<^sub>1 "[init_symbol M]" _ w\<^sub>2 "[]"] by auto
qed
...
using assms proof (induction "Suc n" "(Old_st p\<^sub>1, w\<^sub>1, \<alpha>\<^sub>1)" "(New_final :: 'q st_extended, w\<^sub>2, \<alpha>\<^sub>2)" 
  ...
  then show ?case proof (cases n)
    ...
    from 3(3) p\<^sub>2_def w23 a23 have **: "pda.step\<^sub>1 final_to_stack_pda (Old_st p\<^sub>1, w\<^sub>3, \<alpha>\<^sub>3) (New_final, w\<^sub>3, \<alpha>\<^sub>3)" by simp
    from 0 have ***: "pda.stepn final_to_stack_pda (n - 0) (New_final, w\<^sub>3, \<alpha>\<^sub>3) (New_final, w\<^sub>3, \<alpha>\<^sub>3)"
      using pda.refl\<^sub>n[OF pda_final_to_stack] by simp
    from p\<^sub>1_final * ** *** show ?thesis by blast
  next
    case (Suc n')
    then show ?thesis proof (cases "p\<^sub>2 = New_final")
      ...
      from rule have w23: "w\<^sub>3 = w\<^sub>2"
        by (induction "New_final :: 'q st_extended" Z' rule: final_to_stack_eps_fun.induct) auto
      from k_less Suc have *: "k \<le> n" by simp
      from stepn w23 have **: "pda.stepn final_to_stack_pda k (Old_st p\<^sub>1, w\<^sub>1, \<alpha>\<^sub>1) (Old_st q, w\<^sub>3, \<gamma>)" by simp
      from step1 w23 have ***: "pda.step\<^sub>1 final_to_stack_pda (Old_st q, w\<^sub>3, \<gamma>) (New_final, w\<^sub>3, \<gamma>)" by simp
      from stepn' 3(3) True w23 Suc k_less have ****: 
       "pda.stepn final_to_stack_pda (n - k) (New_final, w\<^sub>3, \<gamma>) (New_final, w\<^sub>3, \<alpha>\<^sub>3)"
        using pda.step\<^sub>n[OF pda_final_to_stack] by (simp add: Suc_diff_le)
      from * q_final ** *** **** show ?thesis by blast
    next
      case False
      ...
      from 3(3) p\<^sub>2_def w23 a23 have **: "pda.step\<^sub>1 final_to_stack_pda (Old_st p\<^sub>2', w\<^sub>3, \<alpha>\<^sub>3) (New_final, w\<^sub>3, \<alpha>\<^sub>3)" by simp
      have ***: "pda.stepn final_to_stack_pda 0 (New_final, w\<^sub>3, \<alpha>\<^sub>3) (New_final, w\<^sub>3, \<alpha>\<^sub>3)"
        using pda.refl\<^sub>n[OF pda_final_to_stack] by simp
      from p\<^sub>2_final * ** *** show ?thesis by force
    qed
  qed
qed (simp add: assms)
...
proof -
  from assms obtain n' where n_def: "n = Suc n'"
    using pda.stepn_not_refl[OF pda_final_to_stack] gr0_implies_Suc by blast
  with assms have "pda.stepn final_to_stack_pda n' (Old_st (init_state M), w\<^sub>1, [Old_sym (init_symbol M), New_sym])
                                                       (New_final, w\<^sub>2, \<gamma>)"
    using pda.stepn_split_first[OF pda_final_to_stack] final_to_stack_pda_first_step by blast
  then obtain n'' where "n' = Suc n''"
    using pda.stepn_not_refl[OF pda_final_to_stack] gr0_implies_Suc by blast
...
"(\<exists>q \<gamma>. q \<in> final_states M \<and> pda.steps M (init_state M, w, [init_symbol M]) (q, [], \<gamma>)) \<longleftrightarrow>
  (\<exists>q. pda.steps final_to_stack_pda (init_state final_to_stack_pda, w, [init_symbol final_to_stack_pda]) (q, [], []))"
proof
  assume "\<exists>q \<gamma>. q \<in> final_states M \<and> pda.steps M (init_state M, w, [init_symbol M]) (q, [], \<gamma>)"
  ...
  from steps have *: "pda.steps final_to_stack_pda (Old_st (init_state M), w, [Old_sym (init_symbol M), New_sym]) 
                                                    (Old_st q, [], \<alpha>_with_new \<gamma>)"
    using final_to_stack_pda_steps by simp
  have **: "pda.step\<^sub>1 final_to_stack_pda (init_state final_to_stack_pda, w, [init_symbol final_to_stack_pda])
  ...
  with map_\<gamma>_def have ***: "pda.step\<^sub>1 final_to_stack_pda (Old_st q, [], \<alpha>_with_new \<gamma>) (New_final, [], Z'#\<alpha>')"
    using pda.step\<^sub>1_rule[OF pda_final_to_stack] by (simp add: final_to_stack_pda_def)
  have ****: "pda.steps final_to_stack_pda (New_final, [], Z'#\<alpha>') (New_final, [], [])"
    using final_to_stack_pda_final_dump by simp
  from * ** *** **** have "pda.steps final_to_stack_pda
                              (init_state final_to_stack_pda, w, [init_symbol final_to_stack_pda]) (New_final, [], [])"
    using pda.step\<^sub>1_stepn_one[OF pda_final_to_stack] pda.stepn_steps[OF pda_final_to_stack]
          pda.steps_trans[OF pda_final_to_stack] by metis
  thus "\<exists>q. pda.steps final_to_stack_pda (init_state final_to_stack_pda, w, [init_symbol final_to_stack_pda]) (q, [], [])" by blast
next
  assume "\<exists>q. pda.steps final_to_stack_pda (init_state final_to_stack_pda, w, [init_symbol final_to_stack_pda]) (q, [], [])"
  ...
  then obtain n' where "n = Suc (Suc n')"
    using final_to_stack_pda_path_length by (metis Suc_le_D Suc_le_mono numeral_2_eq_2) 
  ...
  with p_final show "\<exists>q \<gamma>. q \<in> final_states M \<and> pda.steps M (init_state M, w, [init_symbol M]) (q, [], \<gamma>)"
