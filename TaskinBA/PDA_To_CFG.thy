...
proof -
  let ?A = "(\<lambda>(p, \<alpha>). \<alpha>) ` (\<Union>q a Z. trans_fun M q a Z \<union> (\<Union>q Z. eps_fun M q Z))" 
  have "all_pushes = ?A" by fast

  moreover have "finite ?A" 
    by (rule, simp add: finite_trans finite_eps)+
