...
definition "hd_nts R = {B. \<exists>A w. (A,Nt B # w) \<in> R}"
definition hd_nts_list :: "('n \<times> ('n, 't) sym list) list \<Rightarrow> 'n list" where
"hd_nts_list R = [B. (A,Nt B # w) \<leftarrow> R]"
...
fun hd_in_As where
"hd_in_As As [] = True" |
"hd_in_As As (Tm _ # _) = True" |
"hd_in_As As (Nt B # _) = (B \<in> set As)"
...
*)
fun triangular_list :: "'a list \<Rightarrow> 'a set \<Rightarrow> ('a \<times> ('a, 'b) sym list) list \<Rightarrow> bool" where
